...
namespace FF.DataAccessLayer.BaseDAL
{
    public class BaseDAL
    {
        ...
        //  Task            : Data Access Layer for the logistic system 
        //  Developed By    : Nuwan De Silva
        //  Competed On     : 2017-05-22
        ...
        public Boolean ConnectionOpenUnknown(string _connectionstring)
        {
            ...
            {
                oConnection = new OracleConnection(_connectionstring);
                if (oConnection.State != ConnectionState.Open) oConnection.Open();
                if (oConnection.State == ConnectionState.Open) _isOpen = true;
                else _isOpen = false;
                _isTr = false;
            }
            return _isOpen;
        ...
        public DataTable QueryDataTable(String _tableName, String _storedProcedure, CommandType _commTypes, Boolean _isTransactionBase)
        {
            OracleCommand command = new OracleCommand();
            DataTable table = null;
            MakeCommand(command, oConnection, oTransaction, _commTypes, _storedProcedure, _isTransactionBase);
            OracleDataAdapter adapter = GetAdapter();
            table = new DataTable(_tableName);
            adapter.SelectCommand = command;
            adapter.Fill(table);
        ...
        public Int32 UpdateRecords(String _storedProcedure, CommandType _commTypes)
        {
            OracleCommand command = new OracleCommand();
            Int32 effects = 0;
            MakeCommand(command, oConnection, oTransaction, _commTypes, _storedProcedure, true);
            effects = command.ExecuteNonQuery();
            return effects;
        }
        /// <summary>
