...
jQuery(document).ready(function () {
    ...
    //jQuery("#Cheque_dt").focusout(function () {
    //    
    //    if (new Date("" + jQuery("#Cheque_dt").val() + " 00:00:00 AM") < new Date()) {
    //        setInfoMsg("Check date cannot be backdate");
    //        jQuery('#Cheque_dt').val(my_date_format_tran(my_date_format_with_time(new Date()).toString()));
    //    } 
    //});
    ...
    jQuery(".add-price-amount-btn").on("click", function (evt) {
        ...
        if (Number((jQuery(".bal-amount-val").html().replace(/\,/g, ""))) < Number((jQuery("#Sird_settle_amt").val().replace(/\,/g, "")))) {
            //setInfoMsg("Exceed the pay amount."); // Temporary blocked this by Chathura on 19-nov-2017
            //return;
        }

        if (Number((jQuery("#Sird_settle_amt").val().replace(/\,/g, ""))) != "") {
            
            if (jQuery.isNumeric(Number((jQuery("#Sird_settle_amt").val().replace(/\,/g, ""))))) {
                if (jQuery("#Sird_pay_tp").val() != "") {
                    ...
                    if (payType == "EZCASH" || payType == "STAR_PO") {
                        ...
                        data = {
                            ...
                            addedAmount: Number((jQuery("#Sird_settle_amt").val().replace(/\,/g, ""))),
                    ...
                    if (payType == "CHEQUE" || payType == "TR_CHEQUE") {
                        ...
                        if (jQuery("#Cheque_dt").val() == null || jQuery("#Cheque_dt").val() == "") {
                            
                            setInfoMsg("Check date cannot be blank");
                            //jQuery('#Cheque_dt').val(my_date_format_tran(my_date_format_with_time(new Date()).toString()));
                            return;
                        }
                        if (new Date("" + jQuery("#Cheque_dt").val() + " 00:00:00 AM") < today) {
                            
                            setInfoMsg("Check date cannot be backdate");
                            //jQuery('#Cheque_dt').val(my_date_format_tran(my_date_format_with_time(new Date()).toString()));
                            return;
                        }
                        if (jQuery("#Deposit_bank_cd").val() == null || jQuery("#Deposit_bank_cd").val() == "") {
                            
                            setInfoMsg("Deposit bank cannot be blank");
                            //jQuery('#Cheque_dt').val(my_date_format_tran(my_date_format_with_time(new Date()).toString()));
                            return;
                        }
                    ...
                    if (payType == "DEBT") {
                        if (checkDebitcardValidation() == false) { return; }
                    ...
                    if (payType == "CRCD") {
                        ...
                        if (checkCreditcardValidation() == false) { return;}
                        ...
                        success: function (result) {
                            ...
                            if (result.login == true) {
                                ...
                                if (result.success == true) {
                                    ...
                                    jQuery(".tot-paid-amount-val").empty();
                                    jQuery(".tot-paid-amount-val").html(ReplaceNumberWithCommas(Number(result.totPaid.toString().replace(/[^0-9\.-]+/g, "")).toFixed(2)));
                                    var tot = (jQuery(".tot-amount-val").html() != "") ? jQuery(".tot-amount-val").html() : 0;
                                    var baltemp = (jQuery(".bal-amount-val").html() != "") ? jQuery(".bal-amount-val").html() : 0;
                                    var balamnt = (parseFloat(baltemp.replace(/\,/g, "").toString().replace(/[^0-9\.-]+/g, "") - parseFloat(jQuery("#Sird_settle_amt").val().replace(/\,/g, "")).toString().replace(/[^0-9\.-]+/g, "")));
                                    
                                    jQuery("#Sird_settle_amt").val(ReplaceNumberWithCommas(Number(balamnt).toFixed(2)));
                                    jQuery(".bal-amount-val").empty();
                                    jQuery(".bal-amount-val").html(ReplaceNumberWithCommas(Number(balamnt).toFixed(2)));
                                    ...
                                    var chqno = (typeof data.chequeNum != "undefined") ? data.chequeNum : "";

                                    jQuery('table.payment-table').append('<tr class="new-row">' +
                                                                 '<td>' + ' <button class="btn btn-sm-min btn-red-fullbg remove-pay-mehod-cls">' +
                                   ...
                                   ' </button>' + '</td>' +
                                            ...
                                            '<td>' + crdTyp + '</td>' +
                                            '<td>' + chqno + '</td>' +
                                            '<td>' + ReplaceNumberWithCommas(Number(data.addedAmount.toString().replace(/[^0-9\.-]+/g, "")).toFixed(2)) + '</td>' +
                                            '</tr>');
                                    jQuery(".remove-pay-mehod-cls").unbind('click').click(function (evt) {
                                        ...
                                        var amt = jQuery(td).closest("tr").find('td:eq(6)').text();
                                            ...
                                            success: function (result) {
                                                ...
                                                if (result.login == true) {
                                                    if (result.success == true) {
                                                        ...
                                                        jQuery(".tot-paid-amount-val").html(ReplaceNumberWithCommas(Number(result.totPaid.toString().replace(/[^0-9\.-]+/g, "")).toFixed(2)));
                                                        var settleamntval = ReplaceNumberWithCommas(Number(parseFloat(amt.replace(/[^0-9\.-]+/g, ""))).toFixed(2)); //ReplaceNumberWithCommas(Number(parseFloat(totv.replace(/[^0-9\.-]+/g, "")) - parseFloat(result.totPaid.replace(/[^0-9\.-]+/g, ""))).toFixed(2));
                                                        
                                                        jQuery("#Sird_settle_amt").val(settleamntval);
                                                        
                                                        var bamt = ReplaceNumberWithCommas(Number(parseFloat(jQuery(".bal-amount-val").html().replace(/\,/g, "")) + parseFloat(amt.toString().replace(/[^0-9\.-]+/g, ""))).toFixed(2));
                                                        
                                                        jQuery(".bal-amount-val").empty();
                                                        jQuery(".bal-amount-val").html(bamt); //ReplaceNumberWithCommas(Number(parseFloat(totv.replace(/[^0-9\.-]+/g, "")) - ).toFixed(2)));
                                                    } else {
                                                        if (result.type == "Error") {
                                                            setError(result.msg);
                                                        }
                    ...
                    setInfoMsg("Select payment method.");
                }
            } else {
    ...
    //check numbers and decimal  only //// Commented by Chathura on 22-sep-2017
...
function updateCurrencyAmount(amount, customerCode, invNo, type) {
    // Function edited by Chathura on 20-sep-2017
    
    globeCustomer = customerCode;
    globeInvNo = invNo;
    var settleval = ReplaceNumberWithCommas(Number(amount.toString().replace(/\,/g, "").replace(/[^0-9\.-]+/g, "")).toFixed(2));
    

    var balamnt = Number(jQuery(".bal-amount-val").html().toString().replace(/[^0-9\.-]+/g, ""));
    var adingbalance = amount;
    var totbalance = balamnt + amount
    
    if (type == "DEBT") {
        jQuery("#Sird_settle_amt").val(settleval);
        jQuery(".bal-amount-val").empty();
        jQuery(".bal-amount-val").append(ReplaceNumberWithCommas(Number(settleval.toString().replace(/[^0-9\.-]+/g, "")).toFixed(2)));
        jQuery(".tot-amount-val").empty();
        jQuery(".tot-amount-val").append(ReplaceNumberWithCommas(Number(settleval.toString().replace(/[^0-9\.-]+/g, "")).toFixed(2)));

    } else {
        jQuery("#Sird_settle_amt").val(settleval);
        jQuery(".bal-amount-val").empty();
        jQuery(".bal-amount-val").append(ReplaceNumberWithCommas(Number(totbalance.toString().replace(/[^0-9\.-]+/g, "")).toFixed(2)));
        jQuery(".tot-amount-val").empty();
        jQuery(".tot-amount-val").append(ReplaceNumberWithCommas(Number(totbalance.toString().replace(/[^0-9\.-]+/g, "")).toFixed(2)));
    }
}

function checkCreditcardValidation() {
    if (jQuery("#Cred_crd_ref_no").val() == null || jQuery("#Cred_crd_ref_no").val() == "") {
        setInfoMsg("Please enter credit card number.");
        return false;
    }
    else if ($.isNumeric(jQuery("#Cred_crd_ref_no").val())==false) {
        setInfoMsg("Invalid credit card number.");
        return false;
    }
    else if (jQuery("#Cred_crd_ref_no").val().length == 16) {
        setInfoMsg("Credit card should have 16 numbers.");
        return false;
    }
    else if (jQuery("#Cred_crd_batch").val() == null || jQuery("#Cred_crd_batch").val() == "") {
        setInfoMsg("Please enter batch number.");
        return false;
    }
    else {
        return true;
...
function checkDebitcardValidation() {
    if (jQuery("#Debt_crd_no").val() == null || jQuery("#Debt_crd_no").val() == "") {
        setInfoMsg("Please enter credit card number.");
        return false;
    }
    else if ($.isNumeric(jQuery("#Debt_crd_no").val()) == false) {
        setInfoMsg("Invalid debit card number.");
        return false;
    }
    else if (jQuery("#Debt_crd_no").val().length == 16 ) {
        setInfoMsg("Debit card should have 16 numbers.");
        return false;
    }
    else {
        return true;
