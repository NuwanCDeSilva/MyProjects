...
namespace FF.BusinessObjects.Sales
{
    public class MasterItemTax
    {
        ...
        public static MasterItemTax ConvertTotalBusTax(DataRow row)
        {
            return new MasterItemTax
            {
                Mict_act = row["mbit_act"] == DBNull.Value ? false : Convert.ToBoolean(row["mbit_act"]),
                Mict_com = row["mbit_com"] == DBNull.Value ? string.Empty : row["mbit_com"].ToString(),
                Mict_itm_cd = row["mbit_cd"] == DBNull.Value ? string.Empty : row["mbit_cd"].ToString(),
                // Mict_stus = row["MICT_STUS"] == DBNull.Value ? string.Empty : row["MICT_STUS"].ToString(),
                Mict_taxrate_cd = row["mbit_tax_rt_cd"] == DBNull.Value ? string.Empty : row["mbit_tax_rt_cd"].ToString(),
                Mict_tax_cd = row["mbit_tax_cd"] == DBNull.Value ? string.Empty : row["mbit_tax_cd"].ToString(),
                Mict_tax_rate = row["mbit_tax_rt"] == DBNull.Value ? 0 : Convert.ToDecimal(row["mbit_tax_rt"]),
                Mict_effct_dt = row["mbit_effct_dt"] == DBNull.Value ? DateTime.MinValue : Convert.ToDateTime(row["mbit_effct_dt"])
            };
        }
    }
}
