...
{
    public class PaymentRefundController : BaseController
    {
        // GET: Payment
        ...
        public JsonResult loadPaymenttypes(string payFormType)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(userId) && !string.IsNullOrEmpty(company) && !string.IsNullOrEmpty(userDefPro) && !string.IsNullOrEmpty(userDefLoc))
                {
                    ...
                    if (payTypes != null)
                    {
                        ...
                        {
                            return Json(new { success = false, login = true, msg = "No pay modes found.", type = "Info" }, JsonRequestBehavior.AllowGet);
                        }
            ...
            catch (Exception ex)
            {
                if (!string.IsNullOrEmpty(userId) && !string.IsNullOrEmpty(company) && !string.IsNullOrEmpty(userDefPro) && !string.IsNullOrEmpty(userDefLoc))
                {
                    return Json(new { success = false, login = true, msg = "Server connection error.Please contact system administrator!", type = "Error" }, JsonRequestBehavior.AllowGet);
                }
                else
                {
                    return Json(new { success = false, login = false }, JsonRequestBehavior.AllowGet);
        ...
        public JsonResult getBankCode(string bankId)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(userId) && !string.IsNullOrEmpty(company) && !string.IsNullOrEmpty(userDefPro) && !string.IsNullOrEmpty(userDefLoc))
                {
                    ...
                    if (bank != "")
                    {
                        string bankCode = CHNLSVC.Sales.getBankCode(bankId);
                        if (bankCode != null)
                        {
        ...
        public JsonResult getAdvanceRefAmount(string cuscd, string receiptno)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(userId) && !string.IsNullOrEmpty(company) && !string.IsNullOrEmpty(userDefPro) && !string.IsNullOrEmpty(userDefLoc))
                {
                    ...
                    {
                        cuscd = cuscd.Trim();
                        string amount = CHNLSVC.Sales.getAdvanceRefAmount(cuscd, company, receiptno);
                        if (amount != null)
                        {
                        ...
                        {
                            return Json(new { success = false, login = true, msg = "Invalid reference code.", type = "Info" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
                        return Json(new { success = false, login = true, msg = "Enter reference code.", type = "Info" }, JsonRequestBehavior.AllowGet);
                    }
                }
                else
                {
        ...
        public JsonResult getCreditRefAmount(string cuscd, string receiptno)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(userId) && !string.IsNullOrEmpty(company) && !string.IsNullOrEmpty(userDefPro) && !string.IsNullOrEmpty(userDefLoc))
                {
                    ...
                    cuscd = cuscd.Trim();
                    string amount = CHNLSVC.Sales.getCreditRefAmount(cuscd, company, receiptno, userDefPro);
                    if (amount != null)
                    {
                        return Json(new { success = true, login = true, data = amount }, JsonRequestBehavior.AllowGet);
                    }
            ...
            try
            {
                if (!string.IsNullOrEmpty(userId) && !string.IsNullOrEmpty(company) && !string.IsNullOrEmpty(userDefPro) && !string.IsNullOrEmpty(userDefLoc))
                {
                    ...
                    if (voucherPageNo != "" && isNumeric)
                    {
                        ...
                        List<GiftVoucherPages> _Allgv = CHNLSVC.Sales.GetGiftVoucherPages(null, Convert.ToInt32(voucherPageNo));
                        if (_Allgv.Count > 0)
                        {
                            if (_Allgv != null)
                            {
                                ...
                                {
                                    DataTable _allCom = CHNLSVC.Sales.GetGVAlwCom(company, _tmp.Gvp_gv_cd, 1);
                        ...
                        {
                            return Json(new { success = false, login = true, msg = "Invalid voucher no.", type = "Info" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
            ...
            try
            {
                if (!string.IsNullOrEmpty(userId) && !string.IsNullOrEmpty(company) && !string.IsNullOrEmpty(userDefPro) && !string.IsNullOrEmpty(userDefLoc))
                {
                    ...
                    if (isNumeric)
                    {
                        ...
                        decimal totPaidAmount = 0.00m;
                        decimal currentBalanceAmount = 0.00m;
                        decimal currnetPayAmount = 0.00m;

                        if (paidAmountString == null || paidAmountString == "")
                        {
                            totPaidAmount = Convert.ToDecimal(addedAmount);
                            currentBalanceAmount = (TotalAmount - totPaidAmount);                            
                            currnetPayAmount = Convert.ToDecimal(addedAmount);
                        }
                        else
                        {
                            totPaidAmount = Convert.ToDecimal(paidAmountString);
                            currentBalanceAmount = (TotalAmount - totPaidAmount);
                            currnetPayAmount = Convert.ToDecimal(addedAmount);
                        }
                                ...
                                else
                                {
                                    if (!string.IsNullOrEmpty(customer))
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (!has)
                                            {
                                                //if (currentBalanceAmount >= currnetPayAmount) //// Changed to below by Chathura
                                                if (CheckExceededPayment(currnetPayAmount, currentBalanceAmount))
                                                {
                                                    _item.Sird_deposit_bank_cd = depositBank;
                                                    _item.Sird_deposit_branch = depositBranch;
                                                    _item.Sird_inv_no = invoiceNo;
                                    ...
                                    else
                                    {
                                        ...
                                        if (!has)
                                        {
                                            ...
                                            if (CheckExceededPayment(currnetPayAmount, currentBalanceAmount))
                                            {
                                                ...
                                                totPaidAmount = RecieptItemList.Sum(r => r.Sird_settle_amt);
                                                Session["totalPaidAmount"] = totPaidAmount;
                                                currentBalanceAmount = (TotalAmount - totPaidAmount); // Added by Chathura on 23-sep-2017
                                                return Json(new { success = true, login = true, totPaid = totPaidAmount.ToString() }, JsonRequestBehavior.AllowGet);
                                            }
                                            else
                                            {
            ...
            try
            {
                if (!string.IsNullOrEmpty(userId) && !string.IsNullOrEmpty(company) && !string.IsNullOrEmpty(userDefPro) && !string.IsNullOrEmpty(userDefLoc))
                {
                    ...
                    if (isNumeric)
                    {
                        ...
                        List<GiftVoucherPages> _gift = new List<GiftVoucherPages>();
                        List<GiftVoucherPages> _Allgv = CHNLSVC.Sales.GetGiftVoucherPages(null, Convert.ToInt32(voucherNo));
                        ...
                        if (_gift != null && _gift.Count > 0)
                        {
                            if (_gift.Count == 1)
                            {
                                ...
                                _itemdetail = CHNLSVC.Sales.GetItem(company, _gift[0].Gvp_gv_cd);
                                if (_itemdetail != null)
                                {
                            ...
                            else
                            {
                                ...
                                if (voucherBook != "")
                                {
                                    GiftVoucherPages _giftPage = CHNLSVC.Sales.GetGiftVoucherPage(null, "%", giftVouPage.Gvp_gv_cd, Convert.ToInt32(giftVouPage.Gvp_book), Convert.ToInt32(voucherNo), giftVouPage.Gvp_gv_prefix);
                                    ...
                                    MasterItem _itemdetail = new MasterItem();
                                    _itemdetail = CHNLSVC.Sales.GetItem(company, _giftPage.Gvp_gv_cd);
                                    if (_itemdetail != null)
                                    {
                                        if (_itemdetail.MI_CHK_CUST == 1)
                                        {
                ...
                if (!string.IsNullOrEmpty(userId) && !string.IsNullOrEmpty(company) && !string.IsNullOrEmpty(userDefPro) && !string.IsNullOrEmpty(userDefLoc))
                {
                    ...
                    if (isNumeric)
                    {
                        if (!string.IsNullOrEmpty(refNo))
                        {
                            ...
                            {
                                string amount = CHNLSVC.Sales.getAdvanceRefAmount(customer, company, refNo);
                                if (amount != null)
                                {
                                    refAmount = amount;
                                    DataTable _dt = CHNLSVC.Sales.GetReceipt(refNo);
        ...
        //                    if (!string.IsNullOrEmpty(refAmount))
        //                    {
        //                        string amount = CHNLSVC.Sales.getCreditRefAmount(customer, company, refNo, userDefPro);
        //                        if (amount != null)
        //                        {
        //                            refAmount = amount;
        ...
        // Added by Chathura on 23-sep-2017
        private bool CheckExceededPayment(decimal paidAmount, decimal balanceAmount)
        {
            //if (paidAmount > balanceAmount || balanceAmount == 0) return false; else return true;
            return true;
        }

        public JsonResult updateCashPaymentRefund(string payMode, string totAmount, string addedAmount, string depositBank, string depositBranch, string invoiceNo, string customer)
        {
