...
{
    public partial class DisposalEntryNew : Base
    {
        public List<MasterItemStatus> _comItmStsList
        {
            get
            {
                if (Session["_comItmStsListDisp"] != null)
                {
                    return (List<MasterItemStatus>)Session["_comItmStsListDisp"];
                }
                else
                {
                    return new List<MasterItemStatus>();
                }

            }
            set
            {
                Session["_comItmStsListDisp"] = value;
            }
        } 
        public bool _showSearch
        {
            get
            {
                if (Session["_showSearchDisp"] != null)
                {
                    return (bool)Session["_showSearchDisp"];
                }
                else
                {
                    return false;
        ...
        private void ChangeName()
        {
            ...
            if (id != null)
            {
                disposalEntryDiv.InnerHtml = "Disposal Assets";
                allItemDiv.InnerHtml = " All Asset";
                grdDspItms.Columns[2].HeaderText = "Asset";
                ItemDetailsDiv.InnerHtml = "Asset Details";
                ItemCodeDiv.InnerHtml = "Asset Code";
                btnSaveDispItm.Text = "Save Assets";
                grdDspItms.DataBind();            
        ...
        protected void Clear()
        {
            ...
            _comItmStsList = CHNLSVC.General.GetAllStockTypes(Session["UserCompanyCode"].ToString());
            grdPDACurrentJob.DataSource = new int[] { };
        ...
        protected void lbtnAdd_Click(object sender, EventArgs e)
        {
            ...
            DspItm = CHNLSVC.Inventory.GET_DISPOSAL_ITM_LIST_WEB(Convert.ToInt32(seqNo), txtJobNo.Text, false);
            if (!string.IsNullOrEmpty(txtJobNo.Text))
            {
                DisposalHeader jobEntry = CHNLSVC.Inventory.GET_DISPOSAL_JOB_HEADER(Session["UserCompanyCode"].ToString(), Session["UserDefProf"].ToString(), txtJobNo.Text, "ALL");
                if (jobEntry != null)
            ...
            AddItemGrid(txtItemCode.Text, txtSerialI.Text, txtItmDispLoc.Text.ToUpper(), ddlStatus.SelectedItem.ToString(), txtItmcurLoc.Text.ToUpper(), txtQty.Text, DspItm);
        }

        protected void AddItemGrid(string item, string serial, string itmDispLoc, string stusDesc, string Scurloc, string DQty, List<DisposalItem> DspItm, bool _chksts = false)
        {
            ...
            if (!string.IsNullOrEmpty(itmDispLoc))
            {
                ...
                else if (msitem.Mi_is_ser1 == 0)
                {
                    ...
                    if (_tempSer.Count<1)
                    {
                        ReptPickSerials _minesSerObj = new ReptPickSerials();
                    _minesSerObj.Tus_com = Session["UserCompanyCode"].ToString();
                    // _minesSerObj.Tus_base_doc_no = RequestNo;
                    // _minesSerObj.Tus_base_itm_line = _one.Itrs_line_no;
                   // _minesSerObj.Tus_bin = Session["GlbDefaultBin"].ToString();
                    string _defBin = CHNLSVC.Inventory.GetDefaultBinCode(Session["UserCompanyCode"].ToString(), Session["UserDefLoca"].ToString());
                    _minesSerObj.Tus_bin = _defBin;
                    _minesSerObj.Tus_cre_by = Session["UserID"].ToString();
                    _minesSerObj.Tus_cre_dt = DateTime.Now;
                    _minesSerObj.Tus_cross_batchline = 0;
                    _minesSerObj.Tus_cross_itemline = 0;
                    _minesSerObj.Tus_cross_seqno = 0;
                    _minesSerObj.Tus_cross_serline = 0;
                    _minesSerObj.Tus_doc_dt = Convert.ToDateTime(txtDate.Text);
                    _minesSerObj.Tus_doc_no = "N/A";
                    _minesSerObj.Tus_exist_grncom = "N/A";
                    _minesSerObj.Tus_isapp = 1;
                    _minesSerObj.Tus_iscovernote = 1;
                    _minesSerObj.Tus_itm_brand = msitem.Mi_brand;
                    _minesSerObj.Tus_itm_cd = item;
                    _minesSerObj.Tus_itm_desc = msitem.Mi_longdesc;
                    _minesSerObj.Tus_itm_line = 0;
                    _minesSerObj.Tus_itm_model = msitem.Mi_model;
                    _minesSerObj.Tus_itm_stus = stus;
                    _minesSerObj.Tus_loc = Session["UserDefLoca"].ToString();
                    //     _minesSerObj.Tus_new_status = _one.Itrs_nitm_stus;
                    //_minesSerObj.Tus_qty = _one.Itrs_qty;
                    _minesSerObj.Tus_ser_1 = "N/A";
                    _minesSerObj.Tus_ser_2 = "N/A";
                    _minesSerObj.Tus_ser_id = 0;
                    _minesSerObj.Tus_ser_line = 0;
                    _minesSerObj.Tus_session_id = Session["SessionID"].ToString();
                    _minesSerObj.Tus_unit_cost = 0;
                    _minesSerObj.Tus_unit_price = 0;
                    //  _minesSerObj.Tus_usrseq_no = generated_seq;
                    _minesSerObj.Tus_warr_no = "N/A";
                    _minesSerObj.Tus_warr_period = 0;
                    _tempSer.Add(_minesSerObj);
                    }
                    #endregion
                    ...
                    if (nonSerDspItm != null)
                    {
                        if (nonSerDspItm.Count > 0)
                        {
                            ...
                            nonSerDspItm.FirstOrDefault().Idd_qty += Convert.ToDecimal(DQty);
                            _dispItmList.Where(x => x.Idd_itm_cd == item && x.Idd_stus_desc == stusDesc && x.Idd_cur_loc == Scurloc).FirstOrDefault().Idd_qty = nonSerDspItm.FirstOrDefault().Idd_qty;
                            _dispItmList.Where(x => x.Idd_itm_cd == item && x.Idd_stus_desc == stusDesc && x.Idd_cur_loc == Scurloc).FirstOrDefault().Idd_bqty = nonSerDspItm.FirstOrDefault().Idd_qty;
                            _tempSer2.FirstOrDefault().Tus_qty = Convert.ToDecimal(nonSerDspItm.FirstOrDefault().Idd_qty);
                            TempItmDet_list.Add(_tempSer2.FirstOrDefault());
                ...
                if (TempItmDet_list.Count == 1)
                {
                    ...
                    if (nonSerExist != null)
                    {
                        ...
                        else
                        {
                           ...
                           // DspItm = CHNLSVC.Inventory.GET_DISPOSAL_ITM_LIST_WEB(Convert.ToInt32(seqNo.Value), txtJobNo.Text, false);
                            if (DspItm != null)
                            {
                            ...
                            foreach (ReptPickSerials ser in TempItmDet_list)
                            {
                                ...
                                dispItm.Idd_ser_2 = string.IsNullOrEmpty(ser.Tus_ser_2) ? "N/A" : ser.Tus_ser_2;
                                dispItm.Idd_ser_id = ser.Tus_ser_id;
                                if (ser.Tus_new_status !=null)
                                {
        ...
        protected void btnChngStus_Click(object sender, EventArgs e)
        {
            ...
            if (CHNLSVC.Security.Is_OptionPerimitted(Session["UserCompanyCode"].ToString(), Session["UserID"].ToString(), 16059))
            {
                ...
                if (oItem != null)
                {
                    oItem = oItem.Where(c => c.Idd_stus != "DISPO" && c.Idd_stus != "DISPLP").ToList();
                }
                if (oItem == null)
                {
                ...
                foreach (DisposalItem itm in oItem)
                {
                    ...
                    else if (mstItm.Mi_is_ser1 == 0)
                    {
                        ...
                        #region temp remove if data err in inr_ser and inr_batch
                        //if (_tempSer == null)
                        //{
                           // _tempSer = new ReptPickSerials();
                           // _tempSer.Tus_com = Session["UserCompanyCode"].ToString();
                           //// _tempSer.Tus_bin = Session["GlbDefaultBin"].ToString();
                           // string _defBin = CHNLSVC.Inventory.GetDefaultBinCode(Session["UserCompanyCode"].ToString(), Session["UserDefLoca"].ToString());
                           // _tempSer.Tus_bin = _defBin;
                           // _tempSer.Tus_cre_by = Session["UserID"].ToString();
                           // _tempSer.Tus_cre_dt = DateTime.Now;
                           // _tempSer.Tus_cross_batchline = 0;
                           // _tempSer.Tus_cross_itemline = 0;
                           // _tempSer.Tus_cross_seqno = 0;
                           // _tempSer.Tus_cross_serline = 0;
                           // _tempSer.Tus_doc_dt = Convert.ToDateTime(txtDate.Text);
                           // _tempSer.Tus_doc_no = "N/A";
                           // _tempSer.Tus_exist_grncom = "N/A";
                           // _tempSer.Tus_isapp = 1;
                           // _tempSer.Tus_iscovernote = 1;
                           // _tempSer.Tus_itm_brand = mstItm.Mi_brand;
                           // _tempSer.Tus_itm_cd = itm.Idd_itm_cd;
                           // _tempSer.Tus_itm_desc = mstItm.Mi_longdesc;
                           // _tempSer.Tus_itm_line = 0;
                           // _tempSer.Tus_itm_model = mstItm.Mi_model;
                           // _tempSer.Tus_itm_stus = _outStatus;
                           // _tempSer.Tus_loc = itm.Idd_cur_loc;
                           // //  _tempSer.Tus_new_status = _one.Itrs_nitm_stus;
                           // //  _tempSer.Tus_qty = _one.Itrs_qty;
                           // _tempSer.Tus_ser_1 = "N/A";
                           // _tempSer.Tus_ser_2 = "N/A";
                           // _tempSer.Tus_ser_id = 0;
                           // _tempSer.Tus_ser_line = 0;
                           // _tempSer.Tus_session_id = Session["SessionID"].ToString();
                           // _tempSer.Tus_unit_cost = 0;
                           // _tempSer.Tus_unit_price = 0;
                           // // _tempSer.Tus_usrseq_no = generated_seq;
                           // _tempSer.Tus_warr_no = "N/A";
                           // _tempSer.Tus_warr_period = 0;
                       // }
                        #endregion
                        if (_tempSer != null)
                        {
                    ...
                    {
                        //ReptPickSerials _tempSer = CHNLSVC.Inventory.GetSerialDataForDisposalEntry(Session["UserCompanyCode"].ToString(), itm.Idd_cur_loc, itm.Idd_itm_cd, _outStatus, "N/A", "", 0).FirstOrDefault();
                        ReptPickSerials _tempSer = new ReptPickSerials();
                        _tempSer.Tus_com = Session["UserCompanyCode"].ToString();
                       // _tempSer.Tus_bin = Session["GlbDefaultBin"].ToString();
                        string _defBin = CHNLSVC.Inventory.GetDefaultBinCode(Session["UserCompanyCode"].ToString(), Session["UserDefLoca"].ToString());
                        _tempSer.Tus_bin = _defBin;
        ...
        protected void btnSaveDispItm_Click(object sender, EventArgs e)
        {
            ...
            if (CHNLSVC.Security.Is_OptionPerimitted(Session["UserCompanyCode"].ToString(), Session["UserID"].ToString(), 16034))
            {
                ...
                List<DisposalItem> oDispItems = CHNLSVC.Inventory.GET_DISPOSAL_ITM_LIST_WEB(Convert.ToInt32(seqNo.Value), txtJobNo.Text, false);
                ...
                if (_dispItmList.Count > 0)
                {
                    ...
                    if (_removedItmList.Count > 0)
                    {
                        ...
                        //}
                        //int result2 = CHNLSVC.Inventory.SAVE_DISPOSAL_ITEMS_WEB(_removedItmList, out _err);
                    }
                    // _dispItmList = _dispItmList.Remove(_removedItmList);
                    int result = CHNLSVC.Inventory.SAVE_DISPOSAL_ITEMS_WEB(_dispItmList, out _err);
        ...
        protected void btnSearchSearchPending_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                oFilter.Dh_to_dt = Convert.ToDateTime(txtTo.Text.Trim());
                //oFilter.Dh_doc_no = txtJobNumberPending.Text.Trim();
                List<DisposalHeader> oDisposalHeader = CHNLSVC.Inventory.GET_DISPOSAL_JOBS_DATA(oFilter);
                if (oDisposalHeader != null && oDisposalHeader.Count > 0)
                {
                    oDisposalHeader = oDisposalHeader.OrderBy(x => x.Dh_doc_dt).ToList();
                    foreach (var item in oDisposalHeader)
                    {
                        if (item.Dh_rmk.Length > 50)
                        {
                            item.Tmp_dh_rmk = item.Dh_rmk.Substring(0, 50);
                        }
                        else
                        {
                            item.Tmp_dh_rmk = item.Dh_rmk;
                        }
                    }
                    dgvPendingJobs.DataSource = oDisposalHeader;
                    dgvPendingJobs.DataBind();
                ...
                else
                {
                    ...
                    dgvPendingJobs.DataSource = new int[] { };
                    dgvPendingJobs.DataBind();
                    DisplayMessage("Cannot find jobs");
                    return;
                }
            }
        ...
        protected void btnPendingJObSelect_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                DisplayMessage("Job no : " + txtJobNo.Text + " is loaded successfully!!", 2);
            }
        ...
        protected void btnsend_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                _select = CHNLSVC.Inventory.GET_DISPOSAL_ITM_LIST_WEB(jobEntry.Dh_seq, jobEntry.Dh_doc_no, false);
                if (_select != null)
                {
        ...
        protected void btnDispose_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (_mstItm != null)
                                        {
                                            if (_mstItm.Mi_is_ser1 == -1)
                                            {
                                                _pickSer = new ReptPickSerials();
                                                _pickSer.Tus_qty = Convert.ToDecimal(itm.Idd_qty);
                                                _pickSer.Tus_seq_no = 0;
                                                _pickSer.Tus_itm_line = 0;
                                                _pickSer.Tus_batch_line = 0;
                                                _pickSer.Tus_loc = itm.Idd_disp_loc;
                                                // _pickSer.Tus_ser_line = 0;
                                                _pickSer.Tus_ser_id = 0;
                                                _pickSer.Tus_ageloc = itm.Idd_disp_loc;
                                                _pickSer.Tus_ageloc_dt = DateTime.Now;
                                                _pickSer.Tus_itm_cd = itm.Idd_itm_cd;
                                                _pickSer.Tus_ser_1 = itm.Idd_ser_1;
                                                _pickSer.Tus_ser_line = itm.Idd_line;
                                                _pickSer.Tus_bin = itm.Idd_bin_cd;
                                                _pickSer.Tus_com = Session["UserCompanyCode"].ToString();
                                                _pickSer.Tus_cre_by = Session["UserId"].ToString();
                                                _pickSer.Tus_cre_dt = DateTime.Now;
                                                _pickSer.Tus_itm_stus = "DISPO";
                                                _pickSer.Tus_ser_2 = "N/A";
                                                _tempSer.Add(_pickSer);
                                            }
                ...
                else
                {
                    ...
                    AllDispItem = CHNLSVC.Inventory.GET_DISPOSAL_ITM_LIST_WEB(Convert.ToInt32(seqNo.Value), jobEntry.Dh_doc_no, false);
        ...
        protected void btnupload_Click(object sender, EventArgs e)
        {
            ...
            if (fileupexcelupload.HasFile)
            {
                ...
                if (value == "EmptyStatus")
                {
                    if (string.IsNullOrEmpty(excelErrorValue.Value))
                    {
                        string _ser = excelErrorValue.Value;
                        Label3.Visible = true;
                        Label3.Text = "Status cannot be empty ";
                        excelUpload.Show();
                        excelErrorValue.Value = "";
                        List<DisposalItem> dItmList = CHNLSVC.Inventory.GET_DISPOSAL_ITM_LIST_WEB(Convert.ToInt32(seqNo.Value), txtJobNo.Text, false);
                        //grdDspItms.DataSource = dItmList;
                ...
                if (value == "InvalidSts")
                {
                    if (!string.IsNullOrEmpty(excelErrorValue.Value))
                    {
                        string _ser = excelErrorValue.Value;
                        Label3.Visible = true;
                        Label3.Text = "Status " + _ser + " is invalid";
                        excelUpload.Show();
                        excelErrorValue.Value = "";
                        List<DisposalItem> dItmList = CHNLSVC.Inventory.GET_DISPOSAL_ITM_LIST_WEB(Convert.ToInt32(seqNo.Value), txtJobNo.Text, false);
                        //grdDspItms.DataSource = dItmList;
            ...
            //List<DisposalItem> dItmLists = CHNLSVC.Inventory.GET_DISPOSAL_ITM_LIST_WEB(Convert.ToInt32(seqNo.Value), txtJobNo.Text, false);
            //BindItemGridData(dItmLists);
        }
        ...
        private void ExcelProcess(string FilePath, int option, out string value)
        {
            ...
            if (GetExecelTbl != null)
            {
                if (GetExecelTbl[0].Rows.Count > 0)
                {
                    ...
                    #region add by lakshan 31Oct2017
                    List<MasterItemStatus> _excStsList = new List<MasterItemStatus>();
                    MasterItemStatus _excSts = new MasterItemStatus();
                    #endregion
                    ...
                    for (int i = 1; i < GetExecelTbl[0].Rows.Count; i++)
                    {
                        ...
                        _excSts = new MasterItemStatus();
                        _excSts.Mis_desc = GetExecelTbl[0].Rows[i][2].ToString();
                         if  (!(_excSts.Mis_desc.Equals(string.Empty)))
                         { 
                         _excStsList.Add(_excSts);
                         }
                    ...
                    #region add by lakshan 31Oct2017
                    var _disExcStsList = _excStsList.GroupBy(x => new { x.Mis_desc }).
                        Select(group => new { Peo = group.Key, theCount = group.Count() }).ToList();
                    foreach (var item in _disExcStsList)
                    {
                        if (string.IsNullOrEmpty(item.Peo.Mis_desc))
                        {
                            value = "EmptyStatus";
                            excelErrorValue.Value = item.Peo.Mis_desc;
                            return;
                        }
                        var _stsAva = _comItmStsList.Where(x => x.Mis_desc == item.Peo.Mis_desc).FirstOrDefault();
                        if (_stsAva == null)
                        {
                            value = "InvalidSts";
                            excelErrorValue.Value = item.Peo.Mis_desc;
                        ...
                        var _stsAvaDisp = _currStusList.Where(x => x.Ids_Stus_desc == item.Peo.Mis_desc).FirstOrDefault();
                        if (_stsAvaDisp == null)
                        {
                            value = "InvalidItemSts";
                            excelErrorValue.Value = item.Peo.Mis_desc;
                            return;
                        }
                    }
                        ...
                        try
                        {
                            if (_dtNew.Rows[i][0].ToString().Trim() != "")
                            {
                                ...
                                else if (msitem.Mi_is_ser1 == 0)
                                {
                                    ...
                                    //    _minesSerObj.Tus_itm_line = 0;
                                    //    _minesSerObj.Tus_itm_model = msitem.Mi_model;
                                    //    _minesSerObj.Tus_itm_stus = stusCd;
                                    //    _minesSerObj.Tus_loc = Session["UserDefLoca"].ToString();
                                    //    //     _minesSerObj.Tus_new_status = _one.Itrs_nitm_stus;
                                    //    //_minesSerObj.Tus_qty = _one.Itrs_qty;
                                    //    _minesSerObj.Tus_ser_1 = "N/A";
                                ...
                                AddItemGrid(item, serialI, dspLoc, status, cSLoc, qty, DspItm);
                                if (i == 165)
                                {
                                    int x = 0;
                                }
            ...
            {
                _dispItmList = (List<DisposalItem>)Session["oItem"];
                BindItemGridDataExcel(_dispItmList);
            }
        }
        private void BindItemGridDataExcel(List<DisposalItem> _itmList, bool _shoAll = true)
        {
            
            string _lblTxt = "Document Quantity : 0.00";
            grdDspItms.DataSource = new int[] { };
            if (_itmList != null)
            {
                if (_itmList.Count > 0)
                {
                    _itmList = _itmList.OrderBy(c => c.Idd_itm_cd).ToList();
                    //if (_shoAll)
                    //{
                    //    grdDspItms.DataSource = _itmList;
                    //    _lblTxt = "Document Quantity : " + _itmList.Sum(c => c.Idd_bqty);
                    //}
                    //else
                    //{
                        List<DisposalItem> _tmpItmList = new List<DisposalItem>();
                        for (int i = 0; i < _itmList.Count; i++)
                        {
                            if (i < 201)
                            {
                                _tmpItmList.Add(_itmList[i]);
                            }
                        }
                        if (_tmpItmList.Count > 0)
                        {
                            grdDspItms.DataSource = _tmpItmList;
                            _lblTxt = "Document Quantity : " + _itmList.Sum(c => c.Idd_bqty);
                        }
                    //}
                    //_lblTxt = "Document Quantity : " + _itmList.Sum(c => c.Idd_bqty); 
        ...
        protected void grdDspItms_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            try
            {
                ...
                List<DisposalItem> _tmpList = (List<DisposalItem>)Session["oItem"];
                BindItemGridData(_tmpList);
                grdDspItms.DataBind();
            }
            catch (Exception ex)
            {
        ...
        protected void grdLbtnprint_Click(object sender, EventArgs e)
        {
            ...
            if (row != null)
            {
                ...
                if (lblPrint.Text == "doc_print")
                {
                    ...
                    {
                        CHNLSVC.MsgPortal.SaveReportErrorLog("Disposal Entry Print", "DisposalEntryNew", ex.Message, Session["UserID"].ToString());
                    }
                }
                else if (lblPrint.Text == "ser_print")
                {
                    ...
                    {
                        CHNLSVC.MsgPortal.SaveReportErrorLog("Disposal Entry Serial Print", "DisposalEntryNew", ex.Message, Session["UserID"].ToString());
                    }
                }
            }
            if (success)
        ...
        protected void chkJOB_CheckedChanged(object sender, EventArgs e)
        {
            if (chkJOB.Checked == true)
            {
                ...
                _hdr = _hdr.OrderByDescending(c => c.Tuh_fin_stus).ThenBy(c => c.Tuh_fin_time).ToList();
        ...
        {
            foreach (GridViewRow _row in grdPDACurrentJob.Rows)
            {
                Label lbltus_fin_stus = _row.FindControl("lbltus_fin_stus") as Label;
                if (lbltus_fin_stus.Text == "1")
                {
        ...
        private void BindItemGridData(List<DisposalItem> _itmList, bool _shoAll = false)
        {
            _shoAll = chkAllItem.Checked;
            string _lblTxt = "Document Quantity : 0.00";
            grdDspItms.DataSource = new int[] { };
            if (_itmList != null)
            {
        ...
        protected void chkSelectDoc_CheckedChanged(object sender, EventArgs e)
        {
        ...
        protected void chk_job_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                lblPdaJobQty.Text = "PDA JOB Quantity : 0.00 ";
                var lb = (CheckBox)sender;
                var row = (GridViewRow)lb.NamingContainer;
                CheckBox chk_job = row.FindControl("chk_job") as CheckBox;
                Label col_doc = row.FindControl("col_doc") as Label;
                if (chk_job.Checked)
                {
                    foreach (GridViewRow _row in grdPDACurrentJob.Rows)
                    {
                        CheckBox _chkBox = _row.FindControl("chk_job") as CheckBox;
                        Label _docNo = _row.FindControl("col_doc") as Label;
                        Label lbltus_fin_stus = _row.FindControl("lbltus_fin_stus") as Label;
                        if (_docNo.Text != col_doc.Text)
                        {
        ...
        private void LoadPdaJobQty(string _jobNo)
        {
            lblPdaJobQty.Text = "PDA JOB Quantity : 0.00 ";
            if (!string.IsNullOrEmpty(_jobNo))
            {
                    ReptPickHeader _tmpRptHdr = CHNLSVC.Inventory.GET_TEMP_PICK_HDR_DATA(new ReptPickHeader()
                    {
                        Tuh_doc_no = _jobNo,
                        Tuh_doc_tp = "DISP",
                        Tuh_direct = false,
                    ...
                    if (_tmpRptHdr != null)
                    {
                        List<ReptPickSerials> _repSerList = CHNLSVC.Inventory.GET_TEMP_PICK_SER_DATA(new ReptPickSerials() { Tus_usrseq_no = _tmpRptHdr.Tuh_usrseq_no });
                        if (_repSerList != null)
                        {
                            if (_repSerList.Count > 0)
                            {
                                decimal _pickQty = _repSerList.Sum(c => c.Tus_qty);
                                lblPdaJobQty.Text = "PDA JOB Quantity : " + _pickQty.ToString("###,##0.####");
                            }
                        }
                    }
            }
        }
        protected void JobDispose()
        {
            if (Convert.ToDateTime(txtValidTo.Text) < DateTime.Today)
            {
                DisplayMessage("Valid date is expired!!!");
            ...
            List<DisposalHeader> _dispHdrList = new List<DisposalHeader>();
            List<TmpValidation> _errList = new List<TmpValidation>();
            TmpValidation _err = new TmpValidation();
            List<ReptPickHeader> _rptHdrList = new List<ReptPickHeader>();
            ReptPickHeader _tmpRptHdr = new ReptPickHeader();
            #endregion
            #region get disp hdr data
            foreach (GridViewRow _row in dgvPendingJobs.Rows)
            {
                CheckBox chkSelectDoc = _row.FindControl("chkSelectDoc") as CheckBox;
                LinkButton btnPendingJObSelect = _row.FindControl("btnPendingJObSelect") as LinkButton;
                if (chkSelectDoc.Checked)
                {
                    DisposalHeader _dispHdr = CHNLSVC.Inventory.GET_DISPOSAL_JOB_HEADER(Session["UserCompanyCode"].ToString(),
                        Session["UserDefProf"].ToString(), btnPendingJObSelect.Text.Trim(), "ALL");
                    _dispHdr.DispItemList = CHNLSVC.Inventory.GET_DISPOSAL_ITM_LIST_WEB(0, btnPendingJObSelect.Text.Trim(), true);
                    _dispHdr.RecHdrList = CHNLSVC.Sales.GetRecieptByRefDoc(Session["UserCompanyCode"].ToString(), btnPendingJObSelect.Text.Trim());
                    #region dispos location validate
                    List<DisposalLocation> _dspLocList = CHNLSVC.Inventory.GET_DISPOSAL_LOC_LIST(0, btnPendingJObSelect.Text.Trim());
                    bool isDispLoc = false;
                    var _dispLoc = _dspLocList.Where(c => c.Dl_loc == Session["UserDefLoca"].ToString()).FirstOrDefault();
                    if (_dispLoc != null)
                    {
                        isDispLoc = true;
                    }
                    ...
                    if (_dispHdr != null)
                    {
                        _dispHdrList.Add(_dispHdr);
                    }
                }
            }
            #endregion
            #region get pda job data
            foreach (GridViewRow _row in grdPDACurrentJob.Rows)
            {
                CheckBox chk_job = _row.FindControl("chk_job") as CheckBox;
                Label col_doc = _row.FindControl("col_doc") as Label;
                if (chk_job.Checked)
                {
                    _tmpRptHdr = CHNLSVC.Inventory.GET_TEMP_PICK_HDR_DATA(new ReptPickHeader()
                    {
                        Tuh_doc_no = col_doc.Text,
                        Tuh_doc_tp = "DISP",
                        Tuh_direct = false,
                        Tuh_usr_com = Session["UserCompanyCode"].ToString()
                    }).FirstOrDefault();
                    if (_tmpRptHdr != null)
                    {
                        ...
                        if (_repSerList != null)
                        {
                            _tmpRptHdr.RptSerList = _repSerList;
                            _rptHdrList.Add(_tmpRptHdr);
                        }
                    }
                }
            }
            #endregion
            #region recipt data validate
            foreach (var _dispHdr in _dispHdrList)
            {
                if (_dispHdr.RecHdrList != null)
                {
                    if (_dispHdr.RecHdrList.Count > 0)
                    {
                        decimal pAmt = _dispHdr.RecHdrList.Sum(c => c.Sar_tot_settle_amt);
                        if (pAmt < _dispHdr.Dh_max_val)
                        {
                            DisplayMessage("Customer has not completed the payment");
                            return;
                        }
            ...
            if (chkDspsNotScan.Checked)
            {
                List<ReptPickSerials> rptSerlist = new List<ReptPickSerials>();
                foreach (var _dispHdr in _dispHdrList)
                {
                    foreach (var _disItm in _dispHdr.DispItemList)
                    {
                        _disItm.TMP_idd_res_qty = 0;
                        _disItm.TMP_idd_res_bal = _disItm.Idd_bqty;
                    }
                }
                foreach (var _hdr in _rptHdrList)
                {
                    foreach (var _Ser in _hdr.RptSerList)
                    {
                        _Ser.Tmp_tus_disp_qty = _Ser.Tus_qty;
                    }
                }
                foreach (var _hdr in _rptHdrList)
                {
                    foreach (var _Ser in _hdr.RptSerList)
                    {
                        #region get disp detailes
                        foreach (var _dispHdr in _dispHdrList)
                        {
                            if (_Ser.Tus_isSelect || _Ser.Tmp_tus_disp_qty <= 0)
                            {
                                continue;
                            }
                            var _dItmList = _dispHdr.DispItemList.Where(
                                c => c.Idd_itm_cd == _Ser.Tus_itm_cd && c.Idd_stus == _Ser.Tus_itm_stus && c.TMP_idd_res_bal > 0).ToList();
                            foreach (var _dItm in _dItmList)
                            {
                                if (_Ser.Tus_isSelect || _dItm.TMP_idd_res_bal == 0 || _Ser.Tmp_tus_disp_qty <= 0)
                                {
                                    continue;
                                }
                                if (_Ser.Tmp_tus_disp_qty == _dItm.TMP_idd_res_bal)
                                {
                                    _dItm.TMP_idd_res_qty = _dItm.TMP_idd_res_qty + _Ser.Tmp_tus_disp_qty;
                                    _dItm.TMP_idd_res_bal = 0;
                                    _Ser.Tmp_tus_disp_qty = 0;
                                    _Ser.Tus_base_doc_no = _dItm.Idd_job_no;
                                    _Ser.Tus_base_itm_line = _dItm.Idd_line;
                                    _Ser.Tus_isSelect = true;
                                }
                                if (_Ser.Tmp_tus_disp_qty < _dItm.TMP_idd_res_bal)
                                {
                                    _dItm.TMP_idd_res_qty = _dItm.TMP_idd_res_qty + _Ser.Tmp_tus_disp_qty;
                                    _dItm.TMP_idd_res_bal = _dItm.TMP_idd_res_bal - _Ser.Tmp_tus_disp_qty;
                                    _Ser.Tmp_tus_disp_qty = 0;
                                    _Ser.Tus_base_doc_no = _dItm.Idd_job_no;
                                    _Ser.Tus_base_itm_line = _dItm.Idd_line;
                                    _Ser.Tus_isSelect = true;
                                ...
                                if (_Ser.Tmp_tus_disp_qty > _dItm.TMP_idd_res_bal)
                                {
                                    _dItm.TMP_idd_res_qty = _dItm.TMP_idd_res_qty + _Ser.Tmp_tus_disp_qty;
                                    _dItm.TMP_idd_res_bal = 0;
                                    _Ser.Tmp_tus_disp_qty = _Ser.Tmp_tus_disp_qty - _dItm.TMP_idd_res_bal;
                                    _Ser.Tus_base_doc_no = _dItm.Idd_job_no;
                                    _Ser.Tus_base_itm_line = _dItm.Idd_line;
                                   // _Ser.Tus_isSelect = true;
                                    _Ser.Tus_isSelect = _Ser.Tmp_tus_disp_qty == 0 ? true : false;
                                }
                            }
                        }
                        #endregion
                        if (!_Ser.Tus_isSelect)
                        {
                            string _msg = "Item quantity exceed ! " + _Ser.Tus_itm_cd;
                            DispMsg(_msg); return;
                        }
                        else
                        {
                ...
                List<ReptPickSerialsSub> rptSubSerList = new List<ReptPickSerialsSub>();
                bool IsTemp = false;
                #region Check Duplicate Serials

                if (rptSerlist != null && rptSerlist.Count > 0)
                {
                    var _dup = rptSerlist.Where(x => x.Tus_ser_1 != "N/A" && x.Tus_ser_1 != "").Select(y => y.Tus_ser_id).ToList();
                    ...
                    if (_dup != null)
                        if (_dup.Count > 0)
                            foreach (Int32 _id in _dup)
                            {
                                Int32 _counts = rptSerlist.Where(x => x.Tus_ser_id == _id).Select(y => y.Tus_ser_id).Count();
                                DataTable table2 = new DataTable();
                                using (var reader = ObjectReader.Create(rptSerlist))
                                {
                                    table2.Load(reader);
                                }
                                if (_counts > 1)
                                {
                                    _isDuplicate = true;
                                    var _item = rptSerlist.Where(x => x.Tus_ser_id == _id).Select(x => x.Tus_itm_cd).Distinct();
                                    foreach (string _str in _item)
                                        if (string.IsNullOrEmpty(_duplicateItems))
                                            _duplicateItems = _str;
                                        else
                ...
                inHeader.Ith_vehi_no = string.Empty;
                inHeader.Ith_anal_3 = "";//ddlDeliver.SelectedItem.Text;
                if (_rptHdrList.Count > 0)
                {
                    inHeader.Ith_job_no = _rptHdrList[0].Tuh_doc_no;
                }
                if (_dispHdrList.Count > 0)
                {
                    inHeader.Ith_oth_docno = _dispHdrList[0].Dh_doc_no;
                }
                #endregion
                ...
                string _docNo = "";
                string msg = string.Empty;
                Int32 _res = CHNLSVC.Inventory.DisposalAdjustmentWeb(inHeader, rptSerlist, rptSubSerList, masterAuto, _dispHdrList, out _docNo, out err);
                if (_res > 0)
                {
                    #region after saved
                    msg = "Saved. Disposal Number : " + _docNo + ". Document Number : " + _docNo;
                    Clear();
                    Session["GlbReportType"] = "SCM1_DISPOSAL";
                    ...
                    clsInventory obj = new clsInventory();
                    obj.printOutwardDocs(Session["GlbReportName"].ToString(), _docNo);
                    PrintPDF(targetFileName, obj._outdoc);
        ...
        private void LoadPdaJobGridQty()
        {
            foreach (GridViewRow _row in grdPDACurrentJob.Rows)
            {
                Label col_doc = _row.FindControl("col_doc") as Label;
                Label lblDocPickQty = _row.FindControl("lblDocPickQty") as Label;
                ReptPickHeader _tmpRptHdr = CHNLSVC.Inventory.GET_TEMP_PICK_HDR_DATA(new ReptPickHeader()
                {
                    Tuh_doc_no = col_doc.Text,
                    Tuh_doc_tp = "DISP",
                ...
                if (_tmpRptHdr != null)
                {
                    ...
                    if (_repSerList != null)
                    {
                        ...
                        {
                            decimal _pickQty = _repSerList.Sum(c => c.Tus_qty);
                            lblDocPickQty.Text = _pickQty.ToString("###,##0.00##");
                        }
                    }
                }
            }
        ...
        protected void chkDspsNotScan_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (chkDspsNotScan.Checked)
                {
                    ...
                    List<ReptPickHeader> _rptHdrList = new List<ReptPickHeader>();
                    ReptPickHeader _tmpRptHdr = new ReptPickHeader();
                    List<DisposalItem> _dspItm = new List<DisposalItem>();
                    #endregion
                    #region get disp hdr data
                    foreach (GridViewRow _row in dgvPendingJobs.Rows)
                    {
                    ...
                    foreach (var _hdr in _rptHdrList)
                    {
                        foreach (var _Ser in _hdr.RptSerList)
                        {
                            ...
                            foreach (var _dispHdr in _dispHdrList)
                            {
                                ...
                                var _dItmList = _dispHdr.DispItemList.Where(
                                    c => c.Idd_itm_cd == _Ser.Tus_itm_cd && c.Idd_stus == _Ser.Tus_itm_stus && c.TMP_idd_res_bal > 0 && c.Idd_ser_1 == _Ser.Tus_ser_1).ToList();
                                foreach (var _dItm in _dItmList)
                                {
                                    if (_dItm.Idd_ser_1 == "3TE32M16470401HFVA10332")
                                    {
                                    ...
                                    if (_dItm.Idd_ser_1==_Ser.Tus_ser_1 && _Ser.Tus_ser_1 != "N/A")
                                    {
                                        _dItm.TMP_idd_res_qty = _dItm.TMP_idd_res_qty + _Ser.Tmp_tus_disp_qty;
                                        _dItm.TMP_idd_res_bal = 0;
                                        _Ser.Tmp_tus_disp_qty = 0;
                                        _Ser.Tus_isSelect = true;
                                        _dItm.Idd_scan_stus = 1;
                                        _dItm.Idd_scan_qty = 1;
                                        _dspItm.Add(_dItm);
                                    }
                                    else
                                    {
                                        if (_Ser.Tmp_tus_disp_qty == _dItm.TMP_idd_res_bal)
                                        {
                                            ...
                                            _Ser.Tus_isSelect = true;
                                            _dItm.Idd_scan_stus = 1;
                                            _dItm.Idd_scan_qty = _dItm.TMP_idd_res_qty;
                                            _dspItm.Add(_dItm);
                                        }
                                        if (_Ser.Tmp_tus_disp_qty < _dItm.TMP_idd_res_bal)
                                        {
                                            ...
                                            _dItm.Idd_scan_stus = 1;
                                            _dItm.Idd_scan_qty = _dItm.TMP_idd_res_qty;
                                            var vAva = _dspItm.Where(c => c.Idd_itm_cd == _dItm.Idd_itm_cd && c.Idd_stus == _dItm.Idd_stus).FirstOrDefault();
                                            if (vAva!=null)
                                            {
                                                vAva.Idd_scan_qty = _dItm.Idd_scan_qty;
                                            }
                                            else
                                            {
                                                _dspItm.Add(_dItm);
