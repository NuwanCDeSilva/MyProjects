 ...
 public    class REF_BONUS_HDR
    {
        ...
        public DateTime Rbh_valid_from { get; set; }
        public string Rbh_rerept_opt { get; set; }
        public string RESULT_COUNT { get; set; }
        public string R__ { get; set; }
        public static REF_BONUS_HDR Converter(DataRow row)
        {
            return new REF_BONUS_HDR
            {
                ...
                Rbh_valid_from = row["Rbh_valid_from"] == DBNull.Value ? DateTime.MinValue : Convert.ToDateTime(row["Rbh_valid_from"].ToString()),
                Rbh_rerept_opt = row["Rbh_rerept_opt"] == DBNull.Value ? string.Empty : row["Rbh_rerept_opt"].ToString(),

            };
        }
        public static REF_BONUS_HDR Converter2(DataRow row)
        {
            return new REF_BONUS_HDR
            {
                Rbh_doc_no = row["Rbh_doc_no"] == DBNull.Value ? string.Empty : row["Rbh_doc_no"].ToString(),
                Rbh_anal1 = row["rbh_anal1"] == DBNull.Value ? string.Empty : row["rbh_anal1"].ToString(), // Added by Udesh 12-Oct-2018
