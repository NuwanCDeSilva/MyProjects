...
{
    public partial class ServiceInv : Base
    {
        #region Variables
        ...
        private void LoadInvoiceProfitCenterDetail()
        {
            if (_MasterProfitCenter != null)
                if (_MasterProfitCenter.Mpc_cd != null)
                {
                    ...
                    txtCustomer.Text = _MasterProfitCenter.Mpc_def_customer; txtExecutive.Text = _MasterProfitCenter.Mpc_man; _masterBusinessCompany = CHNLSVC.Sales.GetBusinessCompanyDetail(BaseCls.GlbUserComCode, txtCustomer.Text, string.Empty, string.Empty, "C");
                }
        ...
        private string SetCommonSearchInitialParameters(CommonUIDefiniton.SearchUserControlType _type)
        {
            ...
            switch (_type)
            {
                ...
                case CommonUIDefiniton.SearchUserControlType.ServiceJobSearch:
                    {
                        paramsText.Append(BaseCls.GlbUserComCode + seperator + BaseCls.GlbUserDefLoca + seperator + "2,3,5,6" + seperator);
                        break;
                    }
        ...
        private InvoiceItem AssignDataToObject(bool _isPromotion, MasterItem _item, string _originalItem)
        {
            ...
            _tempItem.Sad_job_no = txtDocRefNo.Text;    //kapila 19/1/2015
        ...
        private void CheckDiscountAmount(object sender, EventArgs e)
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(txtDisAmount.Text)) return;
                this.Cursor = Cursors.WaitCursor;
        ...
        {
            txtDocRefNo_DoubleClick(null, null);
        }

        private void txtDocRefNo_DoubleClick(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;
            CommonSearch.CommonSearchDate _CommonSearch = new CommonSearch.CommonSearchDate();
            ...
            _CommonSearch.dvResult.DataSource = _result;
            _CommonSearch.BindUCtrlDDLData(_result);
            _CommonSearch.obj_TragetTextBox = txtDocRefNo;
            this.Cursor = Cursors.Default;
            _CommonSearch.IsSearchEnter = true;
            _CommonSearch.ShowDialog();
            txtDocRefNo.Focus();
        ...
        private void txtDocRefNo_Leave(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtDocRefNo.Text))
            {
                Service_JOB_HDR OJobheader = CHNLSVC.CustService.GET_SCV_JOB_HDR(txtDocRefNo.Text, BaseCls.GlbUserComCode);
                if (OJobheader != null)
                {
                    txtCusName.Text = OJobheader.SJB_CUST_NAME;
                    txtCustomer.Text = OJobheader.SJB_CUST_CD;
                    txtAddress1.Text = OJobheader.SJB_ADD1;
                    txtAddress2.Text = OJobheader.SJB_ADD2;
                }
                else
                {
                    txtCusName.Text = "";
