...
namespace FastForward.Logistic.Controllers
{
    public class DebitNoteController : BaseController
    {
        ...
        public JsonResult SaveDebitNote(trn_inv_hdr _hdr)
        {
            string userId = HttpContext.Session["UserID"] as string;
            string company = HttpContext.Session["UserCompanyCode"] as string;
            string userDefPro = HttpContext.Session["UserDefProf"] as string;
            ...
            if (!string.IsNullOrEmpty(userId) && !string.IsNullOrEmpty(company) && !string.IsNullOrEmpty(userDefPro) && !string.IsNullOrEmpty(userDefLoc))
            {
                ...
                _hdr.Tih_doc_type = "DEBT";
                _hdr.Tih_inv_tp = "DEBT";
                ...
                _invoiceAuto.Aut_start_char = _hdr.Tih_pc_cd + "-" + _hdr.Tih_inv_tp;
                _invoiceAuto.Aut_year = Convert.ToDateTime(_hdr.Tih_inv_dt).Year;

                int effect = CHNLSVC.Sales.SaveJobDebitNote(_hdr, _invdet, _invoiceAuto, out err);
                if (effect > 0)
                {
