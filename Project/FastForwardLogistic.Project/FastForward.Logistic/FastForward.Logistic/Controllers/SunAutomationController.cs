...
{
    public class SunAutomationController : BaseController
    {
        //
        // GET: /SunUpload/
                                        ...
                                        if (invdata.totalunit != 0)
                                        {
                                            ...
                                            foreach (var gnr in gnrlist)
                                            {
                                                ...
                                                if (paytype == gnrtype && gnr.ledg_sub_tp == "SA" && gnr.ledg_acc_tp == "DR" && gnr.ledg_pc == invdata.sah_pc && (gnr.ledg_desc == "CASH" | gnr.ledg_desc == "CRED") && gnr.ledg_jnl_tp == "15" && gnr.ledg_jnl_desc == invdata.SerCode)
                                                {
                                                    ...
                                                    if (words.Length > 3)
                                                    {
                                                        refNo = words[0] + words[1] + words[2] + words[3];
                                                    }
                                                    ...
                                                    if (invdata.sah_direct == 1)
                                                    {
                                                        if (alltotal != 0)
                                                        {
                                                            ...
                                                            //PrintSunLineInv(Cuscode, TOTNEW2.ToString(), "D", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob);
                                                            string err = "";
                                                            int a = PrintSunLineInv(Cuscode, TOTNEW2.ToString(), "D", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob, out err);

                                                            if(a < 0)
                                                            {
                                                                return Json(new { success = false, login = true, msg = "This is not a sun account code :" + Cuscode, type = "Info" }, JsonRequestBehavior.AllowGet);
                                                            }
                                                        }
                                                    }
                                                    else if (invdata.sah_direct == 0 && invdata.sah_anal_4 == "DISC")
                                                    {
                                                        ...
                                                        if (alltotal != 0)
                                                        {
                                                            ...
                                                            //PrintSunLineInv(Cuscode, TOTNEW2.ToString(), "C", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob);
                                                            string err = "";
                                                            int a = PrintSunLineInv(Cuscode, TOTNEW2.ToString(), "C", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob, out err);
                                                            ...
                                                            foreach (var _vat in LIORDERSUM1)
                                                            {
                                                                ...
                                                                if (_vat.sad_itm_cd == "VAT")
                                                                {
                                                                    ...
                                                                    if (_vat.totalunit != 0)
                                                                    {
                                                                        ...
                                                                        //PrintSunLineInv(Acccode, _tot.ToString(), "C", transdate, refNo, newjob + _vat.tid_cha_desc + invdata.sah_cus_cd.ToString(), pc, seq, jnaltype, execcode, newjob);
                                                                        string err = "";
                                                                        int a = PrintSunLineInv(Acccode, _tot.ToString(), "C", transdate, refNo, newjob +" " + invdata.sah_cus_name.ToString(), pc, seq, jnaltype, execcode, newjob, out err);

                                                                        if (a < 0)
                                                                        {
                                                                            return Json(new { success = false, login = true, msg = "This is not a sun account code :" + Acccode, type = "Info" }, JsonRequestBehavior.AllowGet);
                                                                        }
                                                                    }
                                                                }
                                                            }
                                                ...
                                                if (paytype == gnrtype && gnr.ledg_sub_tp == "SA" && gnr.ledg_acc_tp == "CR" && gnr.ledg_pc == invdata.sah_pc && (gnr.ledg_desc == "CASH" | gnr.ledg_desc == "CRED") && gnr.ledg_jnl_tp == "15" && gnr.ledg_jnl_desc == invdata.SerCode)
                                                {
                                                    ...
                                                    if (invdata.sah_direct == 1)
                                                    {
                                                            ...
                                                            if (_eleacc != "")
                                                            {
                                                                ...
                                                                if (_vat.totalunit != 0 && _vat.sad_itm_cd != "VAT")
                                                                {
                                                                    if (_vat.sad_itm_cd == "NBT" || _vat.sad_itm_cd == "SVAT" || _vat.sad_itm_cd == "COD")
                                                                    {
                                                                        ...
                                                                        //PrintSunLineInv(_eleacc, _tot.ToString(), "C", transdate, refNo, newjob + _vat.tid_cha_desc, pc, seq, jnaltype, execcode, newjob);
                                                                        string err = "";
                                                                        int a = PrintSunLineInv(_eleacc, _tot.ToString(), "C", transdate, refNo, newjob + _vat.tid_cha_desc, pc, seq, jnaltype, execcode, newjob, out err);

                                                                        if (a < 0)
                                                                        {
                                                                            return Json(new { success = false, login = true, msg = "This is not a sun account code :" + _eleacc, type = "Info" }, JsonRequestBehavior.AllowGet);
                                                                        }
                                                                    }
                                                                    else
                                                                    {
                                                                        ...
                                                                        if (_cusacc != "" && _chkdualtax)
                                                                        {
                                                                            ...
                                                                            //PrintSunLineInv(_cusacc, _tot.ToString(), "C", transdate, refNo, newjob + _vat.tid_cha_desc, pc, seq, jnaltype, execcode, newjob);
                                                                            string err = "";
                                                                            int a = PrintSunLineInv(_cusacc, _tot.ToString(), "C", transdate, refNo, newjob + _vat.tid_cha_desc, pc, seq, jnaltype, execcode, newjob, out err);

                                                                            if (a < 0)
                                                                            {
                                                                                return Json(new { success = false, login = true, msg = "This is not a sun account code :" + _cusacc, type = "Info" }, JsonRequestBehavior.AllowGet);
                                                                            }
                                                                        }
                                                                        else
                                                                        {
                                                                            ...
                                                                            //PrintSunLineInv(Acccode, _tot.ToString(), "C", transdate, refNo, newjob + _vat.tid_cha_desc, pc, seq, jnaltype, execcode, newjob);
                                                                            string err = "";
                                                                            int a = PrintSunLineInv(Acccode, _tot.ToString(), "C", transdate, refNo, newjob + _vat.tid_cha_desc, pc, seq, jnaltype, execcode, newjob, out err);
                                                        ...
                                                        if (realvalue != 0)
                                                        {
                                                            ...
                                                            //PrintSunLineInv(Acccode, _aftervat.ToString(), "C", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob);
                                                            string err = "";
                                                            int a = PrintSunLineInv(Acccode, _aftervat.ToString(), "C", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob, out err);
                                                        ...
                                                        if (invdata.isdiliver != "0" || invdata.sah_man_ref.Contains("SRN") == true)
                                                        {
                                                            ...
                                                            if ((cusname == "" || cusname == "N/A" || (cusname.Contains("SRN") == false)))
                                                            {
                                                                ...
                                                                if (invdata.totalunit != 0)
                                                                {
                                                                    ...
                                                                    //PrintSunLineInv(_nbtacc, TOTunit.ToString(), "D", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob);
                                                                    string err = "";
                                                                    int a = PrintSunLineInv(_nbtacc, TOTunit.ToString(), "D", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob, out err);

                                                                    if (a < 0)
                                                                    {
                                                                        return Json(new { success = false, login = true, msg = "This is not a sun account code :" + _nbtacc, type = "Info" }, JsonRequestBehavior.AllowGet);
                                                                    }
                                                                }
                                                            }
                                                            else
                                    ...
                                    foreach (var invdata in listorder)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (invdata.RecieptType == "RTCHQ")
                                            {
                                                ...
                                                string err="";
                                                int a = PrintSunLine(Cuscode, TOTAmmount.ToString(), "D", transdate, "CHQ-" + refNo, refNo, pc, seq, "RTN1", execcode,out err);
                                                if(a < 0)
                                                {
                                                    return Json(new { success = false, login = true, msg = "This is not a sun account code :" + Cuscode, type = "Info" }, JsonRequestBehavior.AllowGet);
                                                }
                                                int b = PrintSunLine(Acccode, TOTAmmount.ToString(), "C", transdate, "CHQ-" + refNo, refNo, pc, seq, "RTN1", execcode, out err);
                                                if (b < 0)
                                                {
                                                    return Json(new { success = false, login = true, msg = "This is not a sun account code :" + Acccode, type = "Info" }, JsonRequestBehavior.AllowGet);
                                                }
                                            ...
                                            else
                                            {
                                                ...
                                                string err = "";
                                                int a = PrintSunLine(Cuscode, TOTAmmount.ToString(), "C", transdate, refNo, cusname, pc, seq, jnaltype, execcode, out err);
                                                if (a < 0)
                                                {
                                                    return Json(new { success = false, login = true, msg = "This is not a sun account code :" + Cuscode, type = "Info" }, JsonRequestBehavior.AllowGet);
                                                }
                                                int b =PrintSunLine(Acccode, TOTAmmount.ToString(), "D", transdate, refNo, cusname, pc, seq, jnaltype, execcode, out err);
                                                if (b < 0)
                                                {
                                                    return Json(new { success = false, login = true, msg = "This is not a sun account code :" + Acccode, type = "Info" }, JsonRequestBehavior.AllowGet);
                                                }
                                    ...
                                    if (_list.Count > 0)
                                    {
                                        foreach (var _petty in _listgrpreq)
                                        {
                                            ...
                                            foreach (var _elilist in _listgrpcd2)
                                            {
                                                ...
                                                //PrintSunLinePetty(_eleacc, setamt.ToString(), "D", transdate, refNo, newjob + _elilist.Customer, pc, seq, jnaltype, execcode, newjob);

                                                string err = "";
                                                int a = PrintSunLinePetty(_eleacc, setamt.ToString(), "D", transdate, refNo, newjob + _elilist.Customer, pc, seq, jnaltype, execcode, newjob, out err);
                                                if(a < 0)
                                                {
                                                    return Json(new { success = false, login = true, msg = "This is not a sun account code :" + _eleacc, type = "Info" }, JsonRequestBehavior.AllowGet);
                                                }
                                            ...
                                            //PrintSunLinePetty(Account, TOTAmmount.ToString(), "C", transdate, refNo, elecode, pc, seq, jnaltype, execcode, newjob);
                                            string err = "";
                                            int b = PrintSunLinePetty(Account, TOTAmmount.ToString(), "C", transdate, refNo, elecode, pc, seq, jnaltype, execcode, newjob, out err);
                                            if (b < 0)
                                            {
                                                return Json(new { success = false, login = true, msg = "This is not a sun account code :" + Account, type = "Info" }, JsonRequestBehavior.AllowGet);
                                            }
                                            }
                                ...
                                if (_list != null)
                                {
                                    if (_list.Count > 0)
                                    {
                                        foreach (var _petty in _list)
                                        {
                                            ...
                                            //PrintSunLinePetty(Account, TOTAmmount.ToString(), "D", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob);
                                            //PrintSunLinePetty(Cuscode, TOTAmmount.ToString(), "C", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob);

                                            string err = "";
                                            int a = PrintSunLinePetty(Account, TOTAmmount.ToString(), "D", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob, out err);
                                            if (a < 0)
                                            {
                                                return Json(new { success = false, login = true, msg = "This is not a sun account code :" + Account, type = "Info" }, JsonRequestBehavior.AllowGet);
                                            }

                                            int b = PrintSunLinePetty(Cuscode, TOTAmmount.ToString(), "C", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob, out err);
                                            if (b < 0)
                                            {
                                                return Json(new { success = false, login = true, msg = "This is not a sun account code :" + Cuscode, type = "Info" }, JsonRequestBehavior.AllowGet);
                                            }
                                ...
                                if (_list != null)
                                {
                                    if (_list.Count > 0)
                                    {
                                        foreach (var _petty in _list)
                                        {
                                            ...
                                            //PrintSunLinePetty(Cuscode, TOTAmmount.ToString(), "D", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob);
                                            //PrintSunLinePetty(Account, TOTAmmount.ToString(), "C", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob);

                                            string err = "";
                                            int a = PrintSunLinePetty(Cuscode, TOTAmmount.ToString(), "D", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob, out err);
                                            if (a < 0)
                                            {
                                                return Json(new { success = false, login = true, msg = "This is not a sun account code :" + Cuscode, type = "Info" }, JsonRequestBehavior.AllowGet);
                                            }

                                            int b = PrintSunLinePetty(Account, TOTAmmount.ToString(), "C", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob, out err);
                                            if (b < 0)
                                            {
                                                return Json(new { success = false, login = true, msg = "This is not a sun account code :" + Account, type = "Info" }, JsonRequestBehavior.AllowGet);
                                            }
...
.Select(cl => new PetyCashUpload
{
    ...
    JobNo = cl.First().JobNo,
                                ...
                                //Added by dilshan on 13/11/2018 for group to job number 
                                List<PetyCashUpload> _listgrpreqjob = _list.GroupBy(l => new { l.tprh_req_no, l.tprh_remarks })
                                .Select(cl => new PetyCashUpload
                                {
                                    tprh_req_no = cl.First().tprh_req_no,
                                    tprh_manual_ref = cl.First().tprh_manual_ref,
                                ...
                                if (_list != null)
                                {
                                    if (_list.Count > 0)
                                    {
                                        foreach (var _petty in _listgrpreq)
                                        {
                                            ...
                                            elecode = newjob + " " + elecode;
                                            //Commented by dilshan on 13/11/2018 for group to job number 
                                            ...
                                            foreach (var _elilist in _llist)
                                            {
                                                ...
                                                //PrintSunLinePetty(_eleacc, setamt.ToString(), "D", transdate, refNo, newjob + _elilist.tprh_remarks, pc, seq, jnaltype, execcode, newjob);
                                                string err = "";
                                                int a = PrintSunLinePetty(_eleacc, setamt.ToString(), "D", transdate, refNo, newjob + _elilist.tprh_remarks, pc, seq, jnaltype, execcode, newjob, out err);
                                                if (a < 0)
                                                {
                                                    return Json(new { success = false, login = true, msg = "This is not a sun account code :" + _eleacc, type = "Info" }, JsonRequestBehavior.AllowGet);
                                                }
                                            ...
                                            {
                                                string err = "";
                                                int b = PrintSunLinePetty(Account, TOTAmmount.ToString(), "C", transdate, refNo, newjob + cusname, pc, seq, jnaltype, execcode, newjob, out err);
                                                if (b < 0)
                                                {
                                                    return Json(new { success = false, login = true, msg = "This is not a sun account code :" + Account, type = "Info" }, JsonRequestBehavior.AllowGet);
                                                }
                                            ...
                                            //PrintSunLinePetty(Account, TOTAmmount.ToString(), "C", transdate, refNo, newjob + cusname, pc, seq, jnaltype, execcode, newjob);
                                            int effect = CHNLSVC.Sales.UPDATE_PETTYREQ(_petty.tprh_req_no, 1, Session["UserCompanyCode"].ToString());
                                        }
                                    }
                                }
     ...
     .Select(cl => new SUNINVHDR
     {
        ...
        .Select(cl => new SUNINVHDR
        {
            ...
            tih_job_no = cl.First().tih_job_no
                                                            ...
                                                            foreach (var _vat in LIORDERSUM1)
                                                            {
                                                                ...
                                                                if (_vat.sad_itm_cd == "VAT")
                                                                {
                                                                    ...
                                                                    if (_vat.totalunit != 0)
                                                                    {
                                                                        ...
                                                                        //PrintSunLineInv(Acccode, _tot.ToString(), "D", transdate, refNo, newjob + _vat.tid_cha_desc + invdata.sah_cus_cd.ToString(), pc, seq, jnaltype, execcode, newjob);
                                                                        string err = "";
                                                                        int a = PrintSunLineInv(Acccode, _tot.ToString(), "D", transdate, refNo, newjob + _vat.tid_cha_desc + invdata.sah_cus_cd.ToString(), pc, seq, jnaltype, execcode, newjob, out err);
                                                ...
                                                if (paytype == gnrtype && gnr.ledg_sub_tp == "SA" && gnr.ledg_acc_tp == "CR" && gnr.ledg_pc == invdata.sah_pc && (gnr.ledg_desc == "CASH" | gnr.ledg_desc == "CRED") && gnr.ledg_jnl_tp == "15" && gnr.ledg_jnl_desc == invdata.SerCode)
                                                {
                                                    ...
                                                    if (invdata.sah_direct == 0)
                                                    {
                                                            ...
                                                            if (_eleacc != "")
                                                            {
                                                                ...
                                                                if (_vat.totalunit != 0 && _vat.sad_itm_cd != "VAT")
                                                                {
                                                                    if (_vat.sad_itm_cd == "NBT" || _vat.sad_itm_cd == "SVAT" || _vat.sad_itm_cd == "COD")
                                                                    {
                                                                        ...
                                                                        //PrintSunLineInv(_eleacc, _tot.ToString(), "D", transdate, refNo, newjob + _vat.tid_cha_desc, pc, seq, jnaltype, execcode, newjob);
                                                                        string err = "";
                                                                        int a = PrintSunLineInv(_eleacc, _tot.ToString(), "D", transdate, refNo, newjob + _vat.tid_cha_desc, pc, seq, jnaltype, execcode, newjob, out err);
                                                                    ...
                                                                    else
                                                                    {
                                                                        ...
                                                                        if (_cusacc != "" && _chkdualtax)
                                                                        {
                                                                            ...
                                                                            //PrintSunLineInv(_cusacc, _tot.ToString(), "D", transdate, refNo, newjob + _vat.tid_cha_desc, pc, seq, jnaltype, execcode, newjob);
                                                                            string err = "";
                                                                            int a = PrintSunLineInv(_cusacc, _tot.ToString(), "D", transdate, refNo, newjob + _vat.tid_cha_desc, pc, seq, jnaltype, execcode, newjob, out err);
                                                                        ...
                                                                        else
                                                                        {
                                                                            ...
                                                                            //PrintSunLineInv(Acccode, _tot.ToString(), "D", transdate, refNo, newjob + _vat.tid_cha_desc, pc, seq, jnaltype, execcode, newjob);
                                                                            string err = "";
                                                                            int a = PrintSunLineInv(Acccode, _tot.ToString(), "D", transdate, refNo, newjob + _vat.tid_cha_desc, pc, seq, jnaltype, execcode, newjob, out err);
                                                        ...
                                                        if (realvalue != 0)
                                                        {
                                                            ...
                                                            //PrintSunLineInv(Acccode, _aftervat.ToString(), "D", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob);
                                                            string err = "";
                                                            int a = PrintSunLineInv(Acccode, _aftervat.ToString(), "D", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob, out err);
                                                        ...
                                                        if (invdata.isdiliver != "0" || invdata.sah_man_ref.Contains("SRN") == true)
                                                        {
                                                            ...
                                                            if ((cusname == "" || cusname == "N/A" || (cusname.Contains("SRN") == false)))
                                                            {
                                                                ...
                                                                if (invdata.totalunit != 0)
                                                                {
                                                                    ...
                                                                    //PrintSunLineInv(_nbtacc, TOTunit.ToString(), "C", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob);
                                                                    string err = "";
                                                                    int a = PrintSunLineInv(_nbtacc, TOTunit.ToString(), "C", transdate, refNo, cusname, pc, seq, jnaltype, execcode, newjob, out err);
                    ...
                    {
                        int effect = CHNLSVC.Sales.UPDATE_JNALNUMBER(Session["UserCompanyCode"].ToString());
                        return Json(new { success = true, login = true, msg = "Successfully Upload " }, JsonRequestBehavior.AllowGet);
                    }
                }
                else
                {
        ...
        //}
        private void PrintSunLine(string AccCode, string Ammount, string AmmountType, DateTime trdate, string refNo, string cusname, string pc, string seq, string jnaltype, string execcode)
        {
            ...
            try
            {
                ...
                ob.entry_date = Convert.ToInt32(trdate.ToString("yyyyMMdd"));
                ob.entry_prd = Convert.ToInt32(summerdate.ToString("yyyy" + "0" + "MM"));
                ob.due_date = 0;
                ob.alloc_ref = 0;
                ob.alloc_date = 0;
                ob.alloc_period = 0;
                ...
                ob.hold_ref = 0;
                ob.hold_op_id = "   ";
                ob.last_change_user_id = HttpContext.Session["UserId"] as string;
                ob.last_change_date = 0;
                ob.originator_id = "   ";
                //ob.ActGRNNo = job;
                ob.Com = HttpContext.Session["UserCompanyCode"] as string;

                int effect = CHNLSVC.Sales.SaveSunAccountsAll(ob);
        ...
        private Int32 PrintSunLine(string AccCode, string Ammount, string AmmountType, DateTime trdate, string refNo, string cusname, string pc, string seq, string jnaltype, string execcode, out string err)
        {
            err = "";
            int effect = 0;
            SunAccountall ob = new SunAccountall();
            try
            {
                ...
                effect = CHNLSVC.Sales.SaveSunAccountsAll(ob);
        ...
        private void PrintSunLinePetty(string AccCode, string Ammount, string AmmountType, DateTime trdate, string refNo, string cusname, string pc, string seq, string jnaltype, string execcode, string job)
        {
            SunAccountall ob = new SunAccountall();
            try
            {
        ...
        private Int32 PrintSunLinePetty(string AccCode, string Ammount, string AmmountType, DateTime trdate, string refNo, string cusname, string pc, string seq, string jnaltype, string execcode, string job, out string err)
        {
            err = "";
            int effect = 0;
            SunAccountall ob = new SunAccountall();
        ...
        public Int32 PrintSunLineInv(string AccCode, string Ammount, string AmmountType, DateTime trdate, string refNo, string cusname, string pc, string seq, string jnaltype, string execcode, string job, out string err)
        {
            err = "";
            int effect = 0;
            try
        ...
        public void PrintSunLineInv(string AccCode, string Ammount, string AmmountType, DateTime trdate, string refNo, string cusname, string pc, string seq, string jnaltype, string execcode, string job)
        {
            try
            {
                SunAccountall ob = new SunAccountall();
