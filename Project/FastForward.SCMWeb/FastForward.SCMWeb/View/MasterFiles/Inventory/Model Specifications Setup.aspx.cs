...
{
    public partial class Model_Specifications_Setup : Base
    {
        ...
        private void Clear()
        {
            ...
            txtVideoRemark.Text = string.Empty;
            txtImgRemark.Text = string.Empty;

            grvideo.DataSource = null;
            grvideo.DataBind();
        ...
        protected void Upload(object sender, EventArgs e)
        {
            if (FileUpload.HasFile)
            {
                try
                {
                    ...
                    if (size > 100)
                    {
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "alert('Image size can not be exceeded 100KB');", true);
                    }
                    ...
                    imagenumber++;
                    fileName = txtModel.Text.Trim() + "[" + imagenumber + ".png";
            ...
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "alert('Please select Image to upload');", true);
            }
        }
        ...
        private void GetmodelDeatils(string _model, string _maincat)
        {
            ...
            Session["_picDetails"] = _picDetails;
            ...
            {
                var _FilterImg = _ModelPic.Where(x => x.MMP_TP == "PIC").ToList();
                foreach (var item in _FilterImg)
                {
                    txtImgRemark.Text = item.MMP_RMK;
                }
                var _FilterVideo = _ModelPic.Where(x => x.MMP_TP == "VIDEO").ToList();
                foreach (var item in _FilterVideo)
                {
                    txtVideoRemark.Text = item.MMP_RMK;
                }
            ...
            try
            {
                ...
                foreach (var item in imagesFolder.EnumerateFiles())
                {
                    if (item is FileInfo)
                    {
                        ...
                        {
                            //add virtual path of the image to the Model list
                            var _result = _ModelPic.Where(x => x.MMP_PATH == (imagesFolder + "\\" + item.Name) && x.MMP_TP == "PIC");
                            string[] tokens2 = tokens[1].ToString().Split('.');
                            imagenumber = Convert.ToInt32(tokens2[0].ToString());
                            if (_result == null)
                            {
                                ...
                                _pic.MMP_TP = "PIC";
                                _pic.MMP_RMK = txtImgRemark.Text;
                            ...
                            {
                                //Record Update
                                foreach (ModelPic _val_img in _ModelPic.Where(x => x.MMP_TP == "PIC"))
                                {
                                    _val_img.MMP_ACT = 1;
                                    _val_img.MMP_CRE_BY = Session["UserID"].ToString();
                                    _val_img.MMP_MOD_BY = Session["UserID"].ToString();
                                    _val_img.MMP_MODEL = txtModel.Text.Trim().ToUpper();
                                    _val_img.MMP_PATH = imagesFolder + "\\" + item.Name;
                                    _val_img.MMP_TP = "PIC";
                                    _val_img.MMP_RMK = txtImgRemark.Text;
                                }
                            }
                        }
                        // Some data record in DB but have not actual img in Physical location
                        else
                        {
                            _ModelPic.Where(c => c.MMP_MODEL == txtModel.Text.Trim().ToUpper()).ToList().ForEach(s => { s.MMP_ACT = 1; s.MMP_CRE_BY = Session["UserID"].ToString(); s.MMP_MOD_BY = Session["UserID"].ToString(); s.MMP_MODEL = txtModel.Text.Trim().ToUpper(); s.MMP_PATH = imagesFolder + "\\" + item.Name; s.MMP_TP = "PIC"; s.MMP_RMK = txtImgRemark.Text; });
                            //_ModelPic.Where(c => c.MMP_MODEL == txtModel.Text.Trim().ToUpper()).ToList().ForEach(s => { s.MMP_ACT = 1; s.MMP_CRE_BY = Session["UserID"].ToString(); s.MMP_MOD_BY = Session["UserID"].ToString(); s.MMP_MODEL = txtModel.Text.Trim().ToUpper(); s.MMP_PATH = imagesFolder + "\\" + item.Name; s.MMP_RMK = txtImgRemark.Text; });
                        }
                    }
                }

                //Add remarks with out img upload
                List<ModelPic> _picVieDtls = Session["_picDetails"] as List<ModelPic>;
                if (_picVieDtls.Count <= 0)
                {
                    ModelPic _pic = new ModelPic();
                    _pic.MMP_ACT = 1;
                    _pic.MMP_CRE_BY = Session["UserID"].ToString();
                    _pic.MMP_MOD_BY = Session["UserID"].ToString();
                    _pic.MMP_MODEL = txtModel.Text.Trim().ToUpper();
                    _pic.MMP_PATH = imagesFolder + "\\" + string.Empty;
                    _pic.MMP_TP = "PIC";
                    _pic.MMP_RMK = txtImgRemark.Text;
                    var _filter = _ModelPic.Where(x => x.MMP_PATH == _pic.MMP_PATH).ToList();
                    if (_filter.Count == 0)
                ...
                if (grvideo.Rows.Count > 0)
                {
                    for (int i = 0; i < grvideo.Rows.Count; i++)
                    {
                        ...
                        var _filter = _ModelPic.Where(x => x.MMP_PATH == path && x.MMP_TP == "VIDEO");
                        ModelPic _pic = new ModelPic();
                        if (_filter == null)
                        {
                            _pic.MMP_MODEL = txtModel.Text;
                            _pic.MMP_ACT = 1;
                            _pic.MMP_CRE_BY = Session["UserID"].ToString();
                            _pic.MMP_MOD_BY = Session["UserID"].ToString();
                            _pic.MMP_MODEL = txtModel.Text.Trim().ToUpper();
                            _pic.MMP_PATH = path;
                            _pic.MMP_TP = "VIDEO";
                            _pic.MMP_RMK = txtVideoRemark.Text;
                            _ModelPic.Add(_pic);
                        }
                            //Update Videos
                        else
                        {
                            foreach (ModelPic val in _ModelPic.Where(r => r.MMP_TP == "VIDEO" && r.MMP_PATH == path))
                            {
                                val.MMP_ACT = 1;
                                val.MMP_MOD_BY = Session["UserID"].ToString();
                                val.MMP_PATH = path;
                                val.MMP_RMK = txtVideoRemark.Text;
                                val.MMP_TP = "VIDEO";
                                val.MMP_MODEL = txtModel.Text.Trim().ToUpper();
                                val.MMP_MOD_DT = DateTime.Now;
                            }
                        }
                        foreach (ModelPic val in _ModelPic.Where(r => r.MMP_TP == "VIDEO" && r.MMP_PATH != path))
                        {
                ...
                {
                    DisplayMessage(err == string.Empty ? "Error while processing Please contact IT Department...!!!" : err, 4);
                }
            }
            catch(Exception ex)
            {
    ...
    && (uriResult.Scheme == Uri.UriSchemeHttp || uriResult.Scheme == Uri.UriSchemeHttps);
                ...
                if (!result)
                {
                    DisplayMessage("Please select valide URL", 1);
                    txtUrl.Text = string.Empty;
                    return;
                }
                if (_ModelPic != null)
        ...
        protected void btnuploadPdf_click(object sender, EventArgs e)
        {
            ...
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "alert('Please select PDF to upload');", true);
            }
        }
    }
}