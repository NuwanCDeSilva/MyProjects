...
namespace FF.WindowsERPClient.UtilityClasses
{
    class NumberToWord
    {
        public string GetWords(string number) {
            ...
            if (parts.Length > 1)
            {
                string whole = ConvertNumberToWord(Convert.ToInt64(parts[0]));
                string cents = ConvertNumberToWord(Convert.ToInt64(parts[1]));

                inWord = whole + " and " + cents + " cents only";
            }
            else {
                inWord = ConvertNumberToWord(Convert.ToInt64(parts[0])) + " only";
            }
            return inWord;
        }

        private  string ConvertNumberToWord(long numberVal)
        {
