jQuery(document).ready(function () {
    ...
    //document.getElementById("divpay").onfocus = function () {
    //    this.blur();
    //};
    ...
    jQuery("#Tih_cus_cd").focusout(function () {
       
        var pc = $(this).val();
        jQuery.ajax({
            cache: false,
            type: "GET",
            url: "/Invoice/GetJobServiceCode",
            data: { jobno: jQuery("#Tih_job_no").val(), cusid: jQuery("#Tih_cus_cd").val() },
            success: function (result) {
                if (result.login == true) {
                    if (result.success == true) {
                  
                        jQuery("#Tid_ser_cd").val(result.service);
                    }
                } else {
                    Logout();
                }
                                ...
                                success: function (result) {
                                    if (result.login == true) {
                                        if (result.success == true) {
                                            ...
                                            if (result.backdates == 1) {
                                                jQuery('#Tih_inv_dt').val(my_date_format(new Date()));
                                                setInfoMsg('Maximum backdate exceeded!');
                                            }
                                            else {
                                                jQuery('#Tih_inv_dt').val(result.invdate);
                                                jQuery('#Tih_inv_dt').attr("readonly", "true");
                                            }
    ...
    function SetInvHdr(data)
    {
        ...
        jQuery("#Tih_inv_amt").val(ReplaceNumberWithCommas(Number(data[0].Tih_inv_amt).toFixed(2))); 
        jQuery(".tot-paid-amount-val").empty();
        var totv = (jQuery(".tot-amount-val").html()) ? jQuery(".tot-amount-val").html() : 0;
        jQuery(".tot-paid-amount-val").html(data[0].Tih_settle_amt);
        jQuery(".bal-amount-val").empty();
        jQuery(".bal-amount-val").html(data[0].Tih_bal_settle_amt);

        if (data[0].Tih_inv_status == "A") {
            jQuery('.btn-approve').attr("disabled", "true");
        }
        else {
            jQuery('.btn-approve').removeAttr("disabled");
    ...
    jQuery('.btn-add-item').click(function (e) {
        ...
        //if (jQuery("#rmk").val() == "") {
        //    setInfoMsg("Remarks cannot be empty");
        //    return;
        //}
        //if (jQuery("#merg").val() == "") {
        //    setInfoMsg("Merge cannot be empty");
        //    return;
        //}
        ...
        if (data.length > 0) {
            ...
            for (i = 0; i < data.length; i++) {
                ...
                jQuery('.tbl-inv-itm').append('<tr class="new-row">' +
                            ...
                            '<td style="width:50px;">' + Number(data[i].Tid_qty).toFixed(2) + '</td>' +
    ...
    function SetPaymentDataList(data) {
        ...
        if (data.length > 0) {
            for (i = 0; i < data.length; i++) {
                jQuery('table.payment-table').append('<tr class="new-row">' +
                     '<td>'+ '</td>' +
                                           '<td>' + data[i].Sard_pay_tp + '</td>' +
                                           '<td>' + data[i].Sard_deposit_bank_cd + '</td>' +
                                           '<td>' + data[i].Sard_deposit_branch + '</td>' +
                                           '<td>' + data[i].Sard_cc_tp + '</td>' +
                                           '<td>' + data[i].Sard_settle_amt + '</td>' +
                                           '</tr>');
            }
        }
    }
    ...
    jQuery('.btn-save-inv').click(function (e) {
        ...
        //}
        //if (jQuery("#Tih_inv_no").val() != "") {
        //    setInfoMsg("Can't Proceed , Already Genarated Number!!");
        //    return;
        //}
        Lobibox.confirm({
            msg: "Do you want to continue process?",
            ...
            callback: function ($this, type, ev) {
                if (type == "yes") {
                    if (jQuery("#Tih_inv_no").val() != "") {
                        jQuery.ajax({
                            type: "GET",
                            url: "/Invoice/validatePrint",
                            data: { Inv: jQuery("#Tih_inv_no").val() },
                            contentType: "application/json;charset=utf-8",
                            dataType: "json",
                            success: function (result) {
                                if (result.login == false) {
                                    ...
                                    if (result.success == true) {
                                        ...
                                        window.open("/Invoice/Print?Inv=" + jQuery("#Tih_inv_no").val() + "&InvType=" + InvType, "_blank");
                                    } else {
                                        if (result.type == "Info") {
                                            setInfoMsg(result.msg);
            ...
            callback: function ($this, type, ev) {
                if (type == "yes") {
                    if (jQuery("#Tih_inv_no").val() != "") {
                            ...
                            success: function (result) {
                                if (result.login == false) {
                                    ...
                                    if (result.success == true) {
                                        ...
                                        window.open("/Invoice/Preview?Inv=" + jQuery("#Tih_inv_no").val() + "&InvType=" + InvType, "_blank");
                                    } else {
                                        if (result.type == "Info") {
                                            setInfoMsg(result.msg);
                                        } else if (result.type == "Error") {
                        ...
                        setInfoMsg("Please select request for preview.");
                    }
                }
            }
        });
    ...
    jQuery('.btn-cancel').click(function (e) {
        ...
        Lobibox.confirm({
            msg: "Do you want to cancel invoice?",
            callback: function ($this, type, ev) {
                if (type == "yes") {
                    ...
                    jQuery.ajax({
                        type: "GET",
                        url: "/Invoice/StatusChangeInvoiceCancel",
                        data: { Invoiceno: jQuery("#Tih_inv_no").val(), Status: "C", invDate: jQuery('#Tih_inv_dt').val() },
                        contentType: "application/json;charset=utf-8",
                        dataType: "json",
                        success: function (result) {
                            if (result.login == true) {
    ...
    jQuery("#Tih_inv_dt").focusout(function () {
        ...
        jQuery.ajax({
            type: "GET",
            url: "/Invoice/InvoiceDateValidate",
            data: { invdate: selected_date },
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            success: function (result) {
                if (result.login == true) {
