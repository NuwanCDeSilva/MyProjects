jQuery(document).ready(function () {
    ...
    if (jQuery("#CostService").length > 0) {
        jQuery.ajax({
            type: "GET",
            url: "/CostingSheet/loadServiceTypes",
    ...
    if (jQuery("#CostCurrency").length > 0) {
        jQuery.ajax({
            type: "GET",
            url: "/CostingSheet/loadCurrencyCode",
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            success: function (result) {
                if (result.login == true) {
    ...
    function CurrencyCodeChange(code, chargeCd, fare, pax, mrkUpMain, service) {
        jQuery.ajax({
            type: "GET",
            url: "/CostingSheet/currencyCodeChange",
            contentType: "application/json;charset=utf-8",
            dataType: "json",
    ...
    function updateChagresGrid(chgCd, service, currencyCode, pax, fare, tax, markup,markupAmt, remarks, totalPax) {
        ...
        if (value == currencyCode) {
            jQuery.ajax({
                type: "GET",
                url: "/CostingSheet/updateTourCharges",
                data: {
                    chgCd: chgCd,
                    service: service,
    ...
    jQuery("#QCH_TOT_PAX").on("input", function () {
        if (jQuery(".cost-sheet-table .new-row").length == 0) {
            ...
            setInfoMsg("You cannot change total pax after adding the charges.");
    ...
    jQuery("#QCH_MARKUP").on("input", function () {
        if (jQuery(".cost-sheet-table .new-row").length > 0) {
            if (jQuery(this).val() != "") {
                ...
                var total = parseFloat(jQuery("#totalCost").val())
                ...
                jQuery("#QCH_TOT_VALUE").val(jQuery("#totalCost").val());
                jQuery("#QuotePerPax").val(parseFloat(jQuery("#totalCost").val()) / parseFloat(parseFloat(jQuery("#QCH_TOT_PAX").val())));
            }
        } else {
    ...
    function codeCusFocusOut(code) {
        if (code != "") {
            jQuery.ajax({
                type: "GET",
                url: "/CostingSheet/cusCodeTextChanged",
                data: { cusCd: code },
                contentType: "application/json;charset=utf-8",
                dataType: "json",
                success: function (result) {
    ...
    jQuery(".btn-costing-clear-data").click(function () {
        Lobibox.confirm({
    ...
    jQuery(".btn-costing-save-data").click(function () {
        Lobibox.confirm({
            msg: "Do you want to save ?",
            callback: function ($this, type, ev) {
                if (type == "yes") {
                    var formdata = jQuery("#costing-crte-frm").serialize();
                    jQuery.ajax({
                        type: "GET",
                        url: "/CostingSheet/saveCostSheet",
                        data: formdata,
                        contentType: "application/json;charset=utf-8",
                        dataType: "json",
                        success: function (result) {
    ...
    jQuery(".btn-cost-enq-approve").click(function () {
            Lobibox.confirm({
                ...
                callback: function ($this, type, ev) {
                    if (type == "yes") {
                        ...
                        jQuery.ajax({
                            type: "GET",
                            url: "/CostingSheet/approveCostingSheet?enqId=" + enqId + "&sendSms=" + sendSms + "&totVal=" + totVal,
                            contentType: "application/json;charset=utf-8",
                            dataType: "json",
                            success: function (result) {
                                if (result.login == true) {
    ...
    jQuery(".btn-cost-enq-reset").click(function () {
        Lobibox.confirm({
            ...
            callback: function ($this, type, ev) {
                if (type == "yes") {
                    ...
                    jQuery.ajax({
                        type: "GET",
                        url: "/CostingSheet/resetCostingSheet?enqId=" + enqId,
                        contentType: "application/json;charset=utf-8",
                        dataType: "json",
                        success: function (result) {
                            if (result.login == true) {
    ...
    jQuery(".btn-print-cost-sheet").click(function () {
        if (jQuery(".cost-sheet-number").html() != "" || jQuery(".cost-sheet-number").html() != "---") {
            if (jQuery("#QCH_OTH_DOC").val() != "") {
                //window.location.href = "/CostingSheet/CostingReport?costNo=" + jQuery(".cost-sheet-number").html() + "&enqId=" + jQuery("#QCH_OTH_DOC").val();
                window.open(
                    "/CostingSheet/CostingReport?costNo=" + jQuery(".cost-sheet-number").html() + "&enqId=" + jQuery("#QCH_OTH_DOC").val(),
            ...
            setInfoMsg("Invalid cost sheet number.");
        }
            ...
            if (jQuery("#CostChargCode").val() != "" && jQuery("#CostPax").val() != "" && jQuery("#CostFare").val() != "") {
                if (jQuery(this).val() != "") {
                    if (parseFloat(jQuery(this).val()) > 0) {
                        var total = parseFloat(jQuery("#CostFare").val()) * parseFloat(jQuery("#CostPax").val()) * parseFloat(jQuery(".currancy-rate-amt").html());
                        total = total + (total * parseFloat(jQuery("#CostTax").val())/100)
                        var markUpPercentage = parseFloat(jQuery(this).val()) * parseFloat(jQuery(".currancy-rate-amt").html()) / total * 100;
                        jQuery("#CostMarkup").val(markUpPercentage.toFixed(4));
                        var totalLkr = total + (parseFloat(jQuery(this).val()) * parseFloat(jQuery(".currancy-rate-amt").html()));
                        jQuery("#CostTotalLkr").val(totalLkr);
                        //jQuery("#QCH_MARKUP").attr("disabled", true);
                    }
                } else {
    ...
    jQuery(".add-emp-cls .btn-cost-genarate-po").click(function () {
        Lobibox.confirm({
            ...
            callback: function ($this, type, ev) {
                if (type == "yes") {
                    ...
                    if (costingSheetNo != "") {
                        jQuery.ajax({
                            type: "GET",
                            url: "/CostingSheet/genaratePurchaseOrder?costingSheetNo=" + costingSheetNo,
                            contentType: "application/json;charset=utf-8",
                            dataType: "json",
                            success: function (result) {
                                if (result.login == true) {
        ...
        Lobibox.confirm({
            ...
            callback: function ($this, type, ev) {
                if (type == "yes") {
                    jQuery.ajax({
                        type: "GET",
                        url: "/CostingSheet/removeChargeItem",
...
function showHideButton(status) {
    jQuery(".btn-costing-save-data").hide();
    jQuery(".btn-cost-enq-reset").hide();
    jQuery(".btn-cost-enq-approve").hide();
    jQuery(".btn-cost-genarate-po").hide();
    jQuery(".btn-print-cost-sheet").hide();
    jQuery(".send-cus-sms").hide();
    if (status == 1) {
        jQuery(".btn-print-cost-sheet").show();
        jQuery(".btn-cost-enq-reset").show();
        jQuery(".btn-cost-genarate-po").show();
    } else if (status == 2) {
        jQuery(".btn-costing-save-data").show();
        jQuery(".btn-cost-enq-approve").show();
        jQuery(".btn-print-cost-sheet").hide();
...
function getCostingSheetEnquiryDetails(enqId) {
    if (enqId != "") {
        jQuery.ajax({
            type: "GET",
            url: "/CostingSheet/getEnquiryDetails?enqId=" + enqId,
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            success: function (result) {
                if (result.login == true) {
...
function setCostingValues(headerDet, itmDet) {
    ...
    if (headerDet != null) {
       ...
       // jQuery("#totalTax").val();
        ...
        jQuery("#QCH_TOT_VALUE").val((headerDet.QCH_TOT_VALUE / parseFloat(itmDet[0].QCD_EX_RATE)).toFixed(4));
        jQuery("#QuotePerPax").val(((headerDet.QCH_TOT_VALUE / parseFloat(itmDet[0].QCD_EX_RATE)) / parseFloat(headerDet.QCH_TOT_PAX)).toFixed(4));
    }
    if (itmDet != null) {
        for (i = 0; i < itmDet.length; i++) {
            ...
            tottax = tottax + parseFloat(itmDet[i].QCD_QTY) *parseFloat(itmDet[i].QCD_TAX) * parseFloat(itmDet[i].QCD_UNIT_COST) / 100;
            totcost = totcost + parseFloat(itmDet[i].QCD_QTY)*parseFloat(itmDet[i].QCD_UNIT_COST);
            totfare = totfare + parseFloat(itmDet[i].QCD_QTY)*parseFloat(itmDet[i].QCD_UNIT_COST);
           // totmarkup = totmarkup + parseFloat(itmDet[i].QCD_MARKUP_AMT);
        }
        totlkr = (tottax + totcost + totfare) * parseFloat(itmDet[0].QCD_EX_RATE);
        jQuery("#totalTax").val((tottax).toFixed(4));
        jQuery("#totalCost").val((totcost + tottax).toFixed(4));
        jQuery("#totalCostLKR").val(totlkr.toFixed(4));
        jQuery("#totalFare").val(totfare.toFixed(4));
        //if (totmarkup !=0)
        //{
        //    jQuery("#QCH_MARKUP").val(totmarkup.toFixed(4));
        //}
    ...
    if (data.length > 0)
    {
        for (i = 0; i < data.length; i++) {
            ...
            jQuery('.cost-sheet-table').append('<tr class="new-row">' +
                   ...
                   '<td style="text-align:center;"><img class="delete-img remove-cost-mehod-cls" src="/Resources/images/Remove.png"></td>' +
                    '</tr>');
            totFareDisplay = parseFloat(totFareDisplay) + parseFloat(data[i].QCD_QTY)*parseFloat(data[i].QCD_UNIT_COST);
            totTaxDisplay = parseFloat(totTaxDisplay) + parseFloat(data[i].QCD_QTY) *parseFloat(data[i].QCD_TAX) * parseFloat(data[i].QCD_UNIT_COST) / 100;
            
            totCostDisplay = parseFloat(totCostDisplay) + parseFloat(data[i].QCD_TOT_COST) + parseFloat(data[i].QCD_MARKUP_AMT);
        ...
        jQuery("#totalCostLKR").val(((totCostDisplay)*parseFloat(jQuery(".currancy-rate-amt").html())).toFixed(4));
        jQuery("#QCH_TOT_VALUE").val((totCostLKRDisplay / parseFloat(jQuery(".currancy-rate-amt").html())).toFixed(4));
        jQuery("#QuotePerPax").val((quotePerPax / parseFloat(jQuery(".currancy-rate-amt").html())).toFixed(4));
