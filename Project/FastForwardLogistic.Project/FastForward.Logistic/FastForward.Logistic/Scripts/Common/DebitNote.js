jQuery(document).ready(function () {
    ...
    jQuery(".inv-no-search").click(function () {
        var headerKeys = Array()
        headerKeys = ["Row", "Debit Note No", "Date"];
        field = "InvoiceNo3"
        var x = new CommonSearch(headerKeys, field);
    });
            ...
            success: function (result) {
                if (result.login == true) {
                    if (result.success == true) {
                        ...
                        setInfoMsg("Debit note data not exists");
                    }
                } else {
                    Logout();
                }
            ...
            callback: function ($this, type, ev) {
                if (type == "yes") {
                    if (jQuery("#Tih_inv_no").val() != "") {
                        jQuery.ajax({
                            type: "GET",
                            url: "/DebitNote/validatePrint?Inv=" + jQuery("#Tih_inv_no").val(),
                            contentType: "application/json;charset=utf-8",
                            dataType: "json",
                            success: function (result) {
                                //if (result.login == false) {
                                ...
                                if (result.login == false) {
                                    ...
                                    if (result.success == true) {
                                        ...
                                        window.open("/Invoice/DebitPrint?Inv=" + jQuery("#Tih_inv_no").val() + "&InvType=" + InvType, "_blank");
                                    } else {
                                        if (result.type == "Info") {
                                            setInfoMsg(result.msg);
                                        } else if (result.type == "Error") {
                            ...
                            success: function (result) {
                                ...
                                if (result.login == false) {
                                    ...
                                    if (result.success == true) {
                                        ...
                                        window.open("/Invoice/DebitPrintPreview?Inv=" + jQuery("#Tih_inv_no").val() + "&InvType=" + InvType, "_blank");
                                    } else {
                                        if (result.type == "Info") {
                                            setInfoMsg(result.msg);
                                        } else if (result.type == "Error") {
