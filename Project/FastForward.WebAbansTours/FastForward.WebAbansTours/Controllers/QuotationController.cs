...
namespace FastForward.WebAbansTours.Controllers
{
    ...
    public class QuotationController : BaseController
    {
        ...
        protected override void Initialize(RequestContext requestContext)
        {
            ...
            {
                ST_MENU per = CHNLSVC.Tours.getAcccessPermission(userId, 1026);
                if (per.MNU_ID == 0)
                {
            ...
            try
            {
                if (!string.IsNullOrEmpty(userId) && !string.IsNullOrEmpty(company) && !string.IsNullOrEmpty(userDefPro) && !string.IsNullOrEmpty(userDefLoc))
                {
                    ...
                    if (!String.IsNullOrEmpty(markup))
                    {
                        ...
                        if (Convert.ToDecimal(markup) > 1000 || Convert.ToDecimal(markup) < 0)
                        {
                    ...
                    if (oItem.QCD_TAX != 0)
                    {
                        ...
                        oItem.QCD_TOT_COST = Math.Round(tot, 0);
                    }
                    else
                    {
                        oItem.QCD_TOT_COST = Math.Round(Convert.ToDecimal(pax) * Convert.ToDecimal(fare), 0);
                    }
                    ...
                    if (markup != "")
                    {
                        oItem.QCD_TOT_LOCAL = Math.Round((Convert.ToDecimal(pax) * Convert.ToDecimal(fare) * _exchangRate) /*+ (Convert.ToDecimal(pax) * Convert.ToDecimal(fare) * _exchangRate) * oItem.QCD_MARKUP / 100*/, 0);
                        oItem.QCD_TOT_LOCAL = oItem.QCD_TOT_LOCAL + oItem.QCD_TOT_LOCAL * oItem.QCD_TAX / 100;
                    }
                    else
                    {
                        oItem.QCD_TOT_LOCAL = Math.Round(Convert.ToDecimal(pax) * Convert.ToDecimal(fare) * _exchangRate, 0);
                        oItem.QCD_TOT_LOCAL = oItem.QCD_TOT_LOCAL + oItem.QCD_TOT_LOCAL * oItem.QCD_TAX / 100;
                    }
                    ...
                    oItem.QCD_AF_MARKUP = Math.Round(oItem.QCD_TOT_LOCAL + (oItem.QCD_TOT_LOCAL * oItem.QCD_MARKUP / 100), 0);
        ...
        public JsonResult getChargSummery() { 
            string userId = HttpContext.Session["UserID"] as string;
            string company = HttpContext.Session["UserCompanyCode"] as string;
            string userDefPro = HttpContext.Session["UserDefProf"] as string;
            string userDefLoc = HttpContext.Session["UserDefLoca"] as string;
            try
            {
                ...
                {
                    oMainItems = (List<QUO_COST_DET>)(Session["oMainItems"]);
                    if (oMainItems != null && oMainItems.Count > 0)
                    {
                        List<summary> sum =new List<summary>();
                        List<MST_COST_CATE> oCate = CHNLSVC.Tours.GET_COST_CATE(company, userDefPro);
                        foreach (MST_COST_CATE service in oCate)
                        {
                            summary one = new summary();
                            one.name=service.MCC_DESC;
                            one.amount = oMainItems.Where(c => c.QCD_CAT == service.MCC_CD).Sum(b => b.QCD_AF_MARKUP).ToString();
                            sum.Add(one);
                        }
                        return Json(new { success = true, login = true,summary=sum }, JsonRequestBehavior.AllowGet);
                    }
                    else {
                        return Json(new { success = false, login = true, msg = "No charges add for display summary.", type = "Info" }, JsonRequestBehavior.AllowGet);
                    }
                }
                else
                {
        ...
        public JsonResult saveQuotation(QUO_COST_HDR formVal)
        {
            string userId = HttpContext.Session["UserID"] as string;
            string company = HttpContext.Session["UserCompanyCode"] as string;
            string userDefPro = HttpContext.Session["UserDefProf"] as string;
                ...
                if (!string.IsNullOrEmpty(userId) && !string.IsNullOrEmpty(company) && !string.IsNullOrEmpty(userDefPro) && !string.IsNullOrEmpty(userDefLoc))
                {
                    ...
                    if (Session["oHeader"] != null)
                    {
                        ...
                        if (oHeader.QCH_ACT != 2) {
                            return Json(new { success = false, login = true, msg = "Unable to save quotation(Invalid status).", type = "Info" }, JsonRequestBehavior.AllowGet);
                        }
                    ...
                    else
                    {
                        ...
                        oHeader.QCH_MARKUP_AMT = oHeader.QCH_TOT_COST_LOCAL * oHeader.QCH_MARKUP/100;
                        oHeader.QCH_TOT_VALUE = Convert.ToDecimal(sumLkr);
                    ...
                    if (result > 0)
                    {
                        return Json(new { success = true, login = true, msg = "Quotation saved successfully. Cost sheet Num : " + err, type = "Success" }, JsonRequestBehavior.AllowGet);
                    }
                    else
                    {
        ...
        public JsonResult approveQuotation(string enqId, string sendSms, string totVal)
        {
            string userId = HttpContext.Session["UserID"] as string;
            string company = HttpContext.Session["UserCompanyCode"] as string;
            string userDefPro = HttpContext.Session["UserDefProf"] as string;
            ...
            try
            {
                if (!string.IsNullOrEmpty(userId) && !string.IsNullOrEmpty(company) && !string.IsNullOrEmpty(userDefPro) && !string.IsNullOrEmpty(userDefLoc))
                {
                    ...
                    {
                        return Json(new { success = false, login = true, msg = "Sorry, You have no permission to approve this quotation.( Advice: Required permission code : 16048) !", type = "Info" }, JsonRequestBehavior.AllowGet);
                    }
                    enqId = enqId.Trim();
                    totVal = totVal.Trim();
                    oHeader = (QUO_COST_HDR)Session["oHeader"];
                    ...
                    if (oHeader.QCH_ACT == 0 || oHeader.QCH_ACT != 2) {
                        return Json(new { success = false, login = true, msg = "Unable to approve quotation(Invalid status)", type = "Info" }, JsonRequestBehavior.AllowGet);
                    }
                    ...
                    {
                        List<RecieptHeaderTBS> RecieptItemList = new List<RecieptHeaderTBS>();
                        RecieptItemList = CHNLSVC.Tours.getReceiptItems(company, userDefPro, "ADVAN", enqId);
                        if (RecieptItemList.Count == 0)
                        {
                            return Json(new { success = false, login = true, msg = "Approve can't proceed without advance payment.", type = "Info" }, JsonRequestBehavior.AllowGet);
                        }
                        else {
                            decimal totSetleAmt = RecieptItemList.Sum(m => m.Sir_tot_settle_amt);
                            if (totSetleAmt == 0)
                            {
                                return Json(new { success = false, login = true, msg = "Approve can't proceed without advance payment.", type = "Info" }, JsonRequestBehavior.AllowGet);
                            }
        ...
        public JsonResult resetQuotation(string enqId)
        {
            string userId = HttpContext.Session["UserID"] as string;
            string company = HttpContext.Session["UserCompanyCode"] as string;
            string userDefPro = HttpContext.Session["UserDefProf"] as string;
            ...
            try
            {
                if (!string.IsNullOrEmpty(userId) && !string.IsNullOrEmpty(company) && !string.IsNullOrEmpty(userDefPro) && !string.IsNullOrEmpty(userDefLoc))
                {
                    ...
                    if (!CHNLSVC.Security.Is_OptionPerimitted(company, userId, 16048))
                    {
                        return Json(new { success = false, login = true, msg = "Sorry, You have no permission to reset this quotation.( Advice: Required permission code : 16048) !", type = "Info" }, JsonRequestBehavior.AllowGet);
                    }
                    enqId = enqId.Trim();
                    oHeader = (QUO_COST_HDR)Session["oHeader"];
                    if (oHeader == null)
                    {
                        return Json(new { success = false, login = true, msg = "Invalid quotation details.", type = "Info" }, JsonRequestBehavior.AllowGet);
                    }
                    if (oHeader.QCH_ACT == 2) {
                        return Json(new { success = false, login = true, msg = "Unable to reset quotation(Invalid status).", type = "Info" }, JsonRequestBehavior.AllowGet);
                    }
                    string err = string.Empty;
        ...
        public JsonResult genaratePurchaseOrder(string costingSheetNo)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(userId) && !string.IsNullOrEmpty(company) && !string.IsNullOrEmpty(userDefPro) && !string.IsNullOrEmpty(userDefLoc))
                {
                    ...
                    if (costingSheetNo == "")
                    {
                        return Json(new { success = false, login = true, msg = "Invalid quotation number.", type = "Info" }, JsonRequestBehavior.AllowGet);
                    }
                    oHeader = (QUO_COST_HDR)Session["oHeader"];
                    if (oHeader == null)
                    {
                        return Json(new { success = false, login = true, msg = "Invalid quotation.", type = "Info" }, JsonRequestBehavior.AllowGet);
                    }
                    oMainItems = (List<QUO_COST_DET>)Session["oMainItems"];
                    if (oMainItems == null)
                    {
                    ...
                    {
                        return Json(new { success = false, login = true, msg = "Unable to find quotation details.", type = "Info" }, JsonRequestBehavior.AllowGet);
                    }
                    if (serpro.Count == 0)
                    {
                ...
                {
                    return Json(new { success = false, login = true, msg = "Invalid quotation item details.", type = "Info" }, JsonRequestBehavior.AllowGet);
                }
