...
{
    public class HpAccountSummary
    {
        Base bsObj;
        public HpAccountSummary()
        {
            bsObj = new Base();
        }
        //HpAccountSummary
        public MasterCompany Get_COMPANY(string comCode)
        {
            return bsObj.CHNLSVC.General.GetCompByCode(comCode);
        }
        public Decimal getAccountBal(string loc, HpAccount Account, DateTime reciptDt)
        {
            return bsObj.CHNLSVC.Sales.Get_AccountBalance(reciptDt, Account.Hpa_acc_no);
        }
        // Get_MonthlyRental
        public Decimal getMonthlyRental(HpAccount Account, DateTime reciptDt)
        {
            return bsObj.CHNLSVC.Sales.Get_MonthlyRental(reciptDt, Account.Hpa_acc_no);
        }
        //Get_FutureRentals
        public Decimal getFutureRentals(HpAccount Account, DateTime reciptDt)
        {
            return bsObj.CHNLSVC.Sales.Get_FutureRentals(reciptDt.Date, Account.Hpa_acc_no);
        }
        public Decimal getAdditionalCommRate(HpAccount Account, DateTime reciptDt, Decimal instComRt)
        {
            return bsObj.CHNLSVC.Sales.Get_hp_additionalCommision(reciptDt, Account.Hpa_sch_cd, instComRt);
        }

        public DataTable getHP_Hierachy(string selectedPC)
        {
            return bsObj.CHNLSVC.Sales.Get_hpHierachy(selectedPC);
        }
        ...
        {
            return bsObj.CHNLSVC.Sales.Get_ECD(ecdTp, date, scheme, partyTP, partyCD, futureRentals, AccNo,IsReduce);
        }
        ...
        {
            return bsObj.CHNLSVC.Sales.Get_hpAcc_TransactionDet(AccNo);
        }
        public Decimal getTotFutureRentalVAl(HpAccount Account, DateTime reciptDt)
        {
            ...
            return bsObj.CHNLSVC.Sales.Get_TotFutureRentalValue(fDateOfNxtMon, Account.Hpa_acc_no);
        ...
        public DataTable getArrearsInfo(string party_tp, string party_cd, DateTime reciptDt)
        {
            ...
            return bsObj.CHNLSVC.Sales.Get_ArrearsInfo(lastDateof_prvMon, party_tp, party_cd);
        }
        ...
        {//------------
            ...
            return bsObj.CHNLSVC.Sales.Get_ArrearsInfo(next_prvMonth, party_tp, party_cd);
        }
        public Decimal getTotDue(string AccNo, DateTime hadd_ars_dt)
        {
            return bsObj.CHNLSVC.Sales.Get_hp_TotalDue(AccNo, hadd_ars_dt.Date);
        }

        public Decimal getDueOnType(string AccNo, DateTime hadd_ars_dt, string type, string receipt_no, DateTime receiptDt)
        {
            return bsObj.CHNLSVC.Sales.Get_hp_TotalDue_onType(AccNo, hadd_ars_dt.Date, type, receipt_no, receiptDt);
        }
        public Decimal getArrearsSettlement(string AccNo, DateTime hadd_sup_dt)
        {
            return bsObj.CHNLSVC.Sales.Get_hp_ArrearsSettlement(AccNo, hadd_sup_dt);
        }
        public Decimal getMinArrears(string party_tp, string party_cd)
        {
            return bsObj.CHNLSVC.Sales.Get_hp_MinArrears(party_tp, party_cd);
        }

        public Decimal getAjustments(string AccNo)
        {
            return bsObj.CHNLSVC.Sales.Get_hp_Adjustment(AccNo);
        }

        public Decimal getTotReceipts(string AccNo, DateTime dt)
        {
            return bsObj.CHNLSVC.Sales.Get_hp_Tot_Receipts(AccNo, dt);
        }
        public string getCustomerName(string AccNo)
        {
            return bsObj.CHNLSVC.Sales.GetHpCustomerName(AccNo);
        }

        public DateTime getEndDate(string AccNo)
        {
            return bsObj.CHNLSVC.Sales.Get_EndingDate(AccNo);
        }

        public Decimal getProtectionPayment_refund(string AccNo)
        {
            return bsObj.CHNLSVC.Sales.Get_ProtectionPayment_RefundValue(AccNo);
        }
        ...
        public static Decimal getArears(string AccNo, HpAccountSummary SUMMARY, string selectedPC, DateTime receiptDate, DateTime HADD_ARS_DT, DateTime HADD_SUP_DT)
        {
            Decimal Arrears = 0;
        ...
        private static Decimal calculateArears(string AccNo, DateTime hadd_ars_dt, DateTime hadd_sup_dt, HpAccountSummary SUMMARY, string selectedPC, out Decimal AllDue)
        {
        ...
        {
            return bsObj.CHNLSVC.Sales.Get_hp_AllDue(AccNo, hadd_ars_dt, hadd_sup_dt);
        }

        //Add By Chamal 19/06/2013
        public HPAccountSummaryValues GetHPAccSummValues(HpAccount _hpAcc, DateTime _receiptDate, string _com)
        {
            return bsObj.CHNLSVC.Sales.GetHPAccSummValues(_hpAcc, _receiptDate, _com);
        }
        public DataTable getCusInvoiceDetails(string CusCode)
        {
            return bsObj.CHNLSVC.Sales.getCusInvoiceDetails(CusCode);
        }
        //Darshana 27-01-2015
        public MasterCompany GetCompanyPara(string _com)
        {
            return bsObj.CHNLSVC.General.GetCompByCode(_com);
        }
        //Darshana 27-01-2015
        public DataTable GetReduceBalECD(DateTime _tDate, string _acc)
        {
            return bsObj.CHNLSVC.Sales.GetDeduceBalEcd(_tDate, _acc);
        }
        //Darshana 27-01-2015
        public DataTable GetDeduceBal(DateTime _fDate, DateTime _tDate, string _com, string _pc, Int32 _isSum, string _acc)
        {
            return bsObj.CHNLSVC.Sales.GetDeduceBal(_fDate, _tDate, _com, _pc, _isSum, _acc);
