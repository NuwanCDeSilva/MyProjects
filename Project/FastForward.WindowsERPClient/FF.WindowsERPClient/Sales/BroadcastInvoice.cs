...
{
    public partial class BroadcastInvoice : Base
    {
        #region Variables
        ...
        public BroadcastInvoice()
        {
            ...
            try
            {
                ...
                cmbInvType.SelectedIndex = 1;   //kapila
                cmbType.SelectedIndex = 0;
                cmbTitle1.SelectedIndex = 7;    //kapila
        ...
        private void TextBoxGotFocus()
        {
            ...
            txtAgeName.GotFocus += txtBox_GotFocus;
            txtAgeAdd1.GotFocus += txtBox_GotFocus;
            txtItmDesc.GotFocus += txtBox_GotFocus;
            txAgeAdd2.GotFocus += txtBox_GotFocus;
        }
        ...
        private void RecallInvoice()
        {
            ...
            if (_hdr == null)
            {
                this.Cursor = Cursors.Default; MessageBox.Show("Please select the valid invoice", "Invoice", MessageBoxButtons.OK, MessageBoxIcon.Information);
                txtInvoiceNo.Text = string.Empty; return;
            }
        ...
        private void LoadInvoiceProfitCenterDetail()
        {
            if (_MasterProfitCenter != null)
                ...
                { if (!_MasterProfitCenter.Mpc_edit_price) txtUnitPrice.ReadOnly = true; txtCustomer.Text = _MasterProfitCenter.Mpc_def_customer; txtExecutive.Text = _MasterProfitCenter.Mpc_man; _masterBusinessCompany = CHNLSVC.Sales.GetBusinessCompanyDetail(BaseCls.GlbUserComCode, txtCustomer.Text, string.Empty, string.Empty, "C"); }
        }
        ...
        private void btnSearch_Customer_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtCustomer.Text == "CASH") //kapila
                {
                    cmbTitle.Text = " ";
                }
            }
        ...
        private void txtAgeName_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                txtAddress2.BackColor = Color.White;
        ...
        private void txtAgeAdd1_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                txtAddress2.BackColor = Color.White;
        ...
        private void txAgeAdd2_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                txtAddress2.BackColor = Color.White;
            ...
            else
            {
                ...
                else if (_priceDetailRef.Count == 1)
                {
                    ...
                    foreach (var _single in _one)
                    {
                        ...
                        {
                            if (_isCombineAdding == false) txtItmDesc.Focus(); 
                        }
        ...
        private InvoiceItem AssignDataToObject(bool _isPromotion, MasterItem _item, string _originalItem)
        {
            ...
            _tempItem.Sad_alt_itm_desc = txtItmDesc.Text;
            ...
            try
            {
                ...
                else if ((chkDeliverLater.Checked == false && chkDeliverNow.Checked == false) || IsGiftVoucher(_itm.Mi_itm_tp) || (_isRegistrationMandatory))
                {
                    if (_itm.Mi_is_ser1 == 1)
                    {
                        ...
                        { this.Cursor = Cursors.Default; { MessageBox.Show("Please select the serial no", "Serial No", MessageBoxButtons.OK, MessageBoxIcon.Information); } txtSerialNo.Focus(); return; }
        ...
        private bool CheckNewDiscountAmount()
        {
            ...
            if (!string.IsNullOrEmpty(txtDisAmt.Text) && _isEditPrice == false && !string.IsNullOrEmpty(txtQty.Text))
            {
                ...
                if(!chkTaxPayable.Checked)
                 _vat = Convert.ToDecimal(txtTaxAmt.Text);
                decimal _totAmt = _uRate * _qty;
                decimal _percent = _totAmt != 0 ? (_disAmt / (_totAmt+_vat)) * 100 : 0;
                ...
                if (_disAmt > 0)
                {
                    if (GeneralDiscount != null)
                    {
                        ...
                        else
                        {
                            ...
                            CalculateItem();
                            if (!string.IsNullOrEmpty(txtDisRate.Text) && Convert.ToDecimal(txtDisRate.Text) == 0) _percent = _totAmt != 0 ? (_disAmt / (_totAmt + _vat)) * 100 : 0;
        ...
        private void SaveWithoutSerial()
        {
            ...
            try
            {
                ...
                _invheader.Sah_man_ref = cmbType.Text;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                _invheader.Sah_cus_cd = txtCustomer.Text.Trim();
                _invheader.Sah_cus_name = txtCusName.Text.Trim();
                _invheader.Sah_d_cust_add1 = txtAgeAdd1.Text.Trim();
                _invheader.Sah_d_cust_add2 = txAgeAdd2.Text.Trim();
                _invheader.Sah_d_cust_cd = txtDelCustomer.Text.Trim();
                _invheader.Sah_d_cust_name = txtAgeName.Text.Trim();
                _invheader.Sah_direct = true;
                _invheader.Sah_dt = Convert.ToDateTime(txtDate.Text);
                _invheader.Sah_epf_rt = 0;
                _invheader.Sah_esd_rt = 0;
               ...
               // _invheader.Sah_man_ref = txtManualRefNo.Text;
                ...
                try
                {
                    ...
                    _invoiceItemList.ForEach(x => x.Sad_srn_qty = 0);
                    List<InvoiceVoucher> _giftVoucher = null;
                    effect = CHNLSVC.Sales.SaveInvoiceDuplicateWithTransaction01(_invheader, _invoiceItemList, InvoiceSerialList, _recHeader, _recieptItem, invHdr, ScanSerialList, null, _invoiceAuto, _receiptAuto, _masterAutoDo,  false, out _invoiceNo, out _receiptNo, out _deliveryOrderNo, _businessEntity, false, _isHoldInvoiceProcess, out _error, _giftVoucher, _buybackheader, _buybackAuto, BuyBackItemList, out _buybackadj, ref IsInvoiceCompleted);
                    //update token
                    if (effect > 0) {
                        if (!string.IsNullOrEmpty(_tokenNo))
                        {
                ...
                finally
                {
                    ...
                    if (effect != -1)
                    {
                        ...
                        if (chkManualRef.Checked == false)
                        {
                            ...
                            { ReportViewer _view = new ReportViewer(); BaseCls.GlbReportName = string.Empty; GlbReportName = string.Empty; _view.GlbReportName = string.Empty; _view.GlbReportName = "InvoicePrints_New1.rpt"; _view.GlbReportDoc = _invoiceNo; _view.Show(); _view = null; _isPrintElite = true; } 
                            ...
                            if (_isPrintElite == false)
                            {
                                if (_itm.Mbe_sub_tp != "C.")
                                {
                                    ...
                                    //========================= INVOCIE  CASH/CREDIT/ HIRE 
                                    if (chkTaxPayable.Checked == false)
                                    { ReportViewer _view = new ReportViewer(); BaseCls.GlbReportName = string.Empty; GlbReportName = string.Empty; _view.GlbReportName = string.Empty; _view.GlbReportName = "InvoicePrints_New1.rpt"; _view.GlbReportDoc = _invoiceNo; _view.Show(); _view = null; }
                                    else
                                    {
                                        //Add Code by Chamal 27/04/2013
                                        //====================  TAX INVOICE
                                        ReportViewer _view = new ReportViewer(); BaseCls.GlbReportName = string.Empty; GlbReportName = string.Empty; _view.GlbReportName = string.Empty; _view.GlbReportName = "InvoicePrints_New1.rpt"; _view.GlbReportDoc = _invoiceNo; _view.Show(); _view = null;
                                        if (_recieptItem != null) if (_recieptItem.Count > 0) if (_itm.Mbe_cate == "LEASE") { ReportViewer _viewt = new ReportViewer(); BaseCls.GlbReportName = string.Empty; GlbReportName = string.Empty; _view.GlbReportName = string.Empty; _viewt.GlbReportName = "InvoicePrints_New1.rpt"; _viewt.GlbReportDoc = _invoiceNo; _viewt.Show(); _viewt = null; }
                                        //====================  TAX INVOICE
                                    }
                                }
                                else
                        ...
                        if (chkDeliverLater.Checked == false || chkDeliverNow.Checked)
                        {
                            ...
                            {
                                if (MessageBox.Show("Do you need to print delivery order now?", "Print", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                                { ReportViewerInventory _views = new ReportViewerInventory(); BaseCls.GlbReportName = string.Empty; GlbReportName = string.Empty; _views.GlbReportName = string.Empty; _views.GlbReportName = BaseCls.GlbUserComCode == "SGL" ? "Outward_Docs.rpt" : BaseCls.GlbDefChannel == "AUTO_DEL" ? "Dealer_Outward_Docs.rpt" : "Outward_Docs.rpt"; _views.GlbReportDoc = _deliveryOrderNo; _views.Show(); _views = null; }
                            }
                            else
                            { ReportViewerInventory _views = new ReportViewerInventory(); BaseCls.GlbReportName = string.Empty; GlbReportName = string.Empty; _views.GlbReportName = string.Empty; _views.GlbReportName = BaseCls.GlbUserComCode == "SGL" ? "Outward_Docs.rpt" : BaseCls.GlbDefChannel == "AUTO_DEL" ? "Dealer_Outward_Docs.rpt" : "Outward_Docs.rpt"; _views.GlbReportDoc = _deliveryOrderNo; _views.Show(); _views = null; }
                        }
                        ...
                        {
                            MasterBusinessEntity _tem = CHNLSVC.Sales.GetBusinessCompanyDetail(BaseCls.GlbUserComCode, txtCustomer.Text, string.Empty, string.Empty, "C");
            ...
            try
            {
                ...
                //else
                {
                    ...
                    if (_one != null && _one.Count > 0 && IsPriceLevelAllowDoAnyStatus == false)
                    {
                        ...
                        {
                            this.Cursor = Cursors.Default;
                            { MessageBox.Show("Selected serial item inventory status not available in price level status collection. Please contact IT dept.", "Inventory Item Status", MessageBoxButtons.OK, MessageBoxIcon.Information); }
                            txtSerialNo.Clear();
                            txtItem.Clear();
                            txtSerialNo.Focus();
                            return;
        ...
        private void txtItmDesc_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter) {
                txtQty.Focus();
            }
        ...
        private void cmbTitle1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                this.Cursor = Cursors.WaitCursor;
                if (string.IsNullOrEmpty(txtAgeName.Text))
                    txtAgeName.Text = cmbTitle1.Text.Trim();
                else
                {
                    string _title = ExtractTitleByCustomerName(txtAgeName.Text.Trim());
                    if (string.IsNullOrEmpty(_title))
                        txtAgeName.Text = cmbTitle.Text.Trim() + txtAgeName.Text;
                    else
                    {
                        bool _isExist = cmbTitle.Items.Contains(_title);
                        if (_isExist)
                        {
                            string _currentCustomerName = txtAgeName.Text.Trim();
                            txtAgeName.Text = _currentCustomerName.Replace(_title.ToUpper(), cmbTitle1.Text.Trim().ToUpper());
                        }
                    }
                }
            }
        ...
        {
            if (cmbType.SelectedItem == "Agency")
            {
                pnlAgent.Enabled = true;
            }
            else
            {
                txtAgeName.Text = "";
                cmbTitle1.SelectedIndex = -1;
                txtAgeAdd1.Text = "";
                txAgeAdd2.Text = "";
                pnlAgent.Enabled = false;
            }
        }
