...
namespace FastForward.WebAbansTours.Controllers
{
    public class ReceiptEntryController : BaseController
    {
        ...
        public JsonResult saveReciptData(RecieptHeaderTBS reciptVal, string Division, string CheckManSys,string isOtherParty)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(userId) && !string.IsNullOrEmpty(company) && !string.IsNullOrEmpty(userDefPro) && !string.IsNullOrEmpty(userDefLoc))
                {
                    ...
                    string err = "";
                    Int32 row_aff = SaveReceiptHeader(reciptVal, Division, CheckManSys, OthSr, OthSrVal,out err);
                    //if (CheckManSys == "1")
                    //{
        ...
        string QTNum;
        private Int32 SaveReceiptHeader(RecieptHeaderTBS receiptVal, string Division, string CheckManSys, string OthSr, string OthSrVal, out string err)
        {
            err="";
            string company = HttpContext.Session["UserCompanyCode"] as string;
            string userDefPro = HttpContext.Session["UserDefProf"] as string;
