...
namespace FF.BusinessObjects.General
{
    public class BMT_INV_BAL_GIT
    {
        ...
        public String BMI_OTH_LOC { get; set; }
        public String BMI_REF_DOC_NO { get; set; }
        public Int32 BMI_LOG_SEQ { get; set; }
        public Int32 BMI_BATCH_LINE { get; set; }
        public Int32 BMI_GIT_CLOSE { get; set; }
        public DateTime BMI_GIT_CLOSE_DT { get; set; }
        public String BMI_SCM_DOC { get; set; }
        public String BMI_GROUP { get; set; }
        public String BMI_ADMIN_TEAM { get; set; }
        public static BMT_INV_BAL_GIT Converter(DataRow row)
        {
            return new BMT_INV_BAL_GIT
            {
                ...
                BMI_OTH_LOC = row["BMI_OTH_LOC"] == DBNull.Value ? string.Empty : row["BMI_OTH_LOC"].ToString(),
                BMI_REF_DOC_NO = row["BMI_REF_DOC_NO"] == DBNull.Value ? string.Empty : row["BMI_REF_DOC_NO"].ToString(),
                BMI_LOG_SEQ = row["BMI_LOG_SEQ"] == DBNull.Value ? 0 : Convert.ToInt32(row["BMI_LOG_SEQ"].ToString()),
                BMI_BATCH_LINE = row["BMI_BATCH_LINE"] == DBNull.Value ? 0 : Convert.ToInt32(row["BMI_BATCH_LINE"].ToString()),
                BMI_GIT_CLOSE = row["BMI_GIT_CLOSE"] == DBNull.Value ? 0 : Convert.ToInt32(row["BMI_GIT_CLOSE"].ToString()),
                BMI_GIT_CLOSE_DT = row["BMI_GIT_CLOSE_DT"] == DBNull.Value ? DateTime.MinValue : Convert.ToDateTime(row["BMI_GIT_CLOSE_DT"].ToString()),
                BMI_SCM_DOC = row["BMI_SCM_DOC"] == DBNull.Value ? string.Empty : row["BMI_SCM_DOC"].ToString()
