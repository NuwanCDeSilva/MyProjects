    ...
    public partial class Base : System.Web.UI.Page
    {
        ...
        public ChannelOperator CHNLSVC
        {
            get
            {
                ...
                //        if (channelServiceTemp.Security.IsSessionExpired(BaseCls.GlbUserSessionID, BaseCls.GlbUserID, Session["UserCompanyName"].ToString()) == true)
                //        {
        ...
        {
            MasterProfitCenter _mstPc = CHNLSVC.Sales.GetProfitCenter(Session["UserCompanyName"].ToString(), BaseCls.GlbUserDefProf);
            if (_mstPc != null)
            {
        ...
        public void LoadProfitCenterDetail()
        {
            ...
            //BaseCls.GlbMasterProfitCenter = CHNLSVC.Sales.GetProfitCenter(Session["UserCompanyName"].ToString(), BaseCls.GlbUserDefProf);
            //BaseCls.GlbPriceDefinitionRef = CHNLSVC.Sales.GetPriceDefinitionByBookAndLevel(Session["UserCompanyName"].ToString(), string.Empty, string.Empty, string.Empty, BaseCls.GlbUserDefProf);
            MasterProfitCenter _ctn = CHNLSVC.Sales.GetProfitCenter(Session["UserCompanyName"].ToString(), BaseCls.GlbUserDefProf);
            CacheLayer.Add(_ctn, CacheLayer.Key.ProfitCenter.ToString());
            CacheLayer.Add(CHNLSVC.Inventory.GetChannelDetail(Session["UserCompanyName"].ToString(), _ctn.Mpc_chnl), CacheLayer.Key.ChannelDefinition.ToString());
            CacheLayer.Add(CHNLSVC.Sales.GetPriceDefinitionByBookAndLevel(Session["UserCompanyName"].ToString(), string.Empty, string.Empty, string.Empty, BaseCls.GlbUserDefProf), CacheLayer.Key.PriceDefinition.ToString());
            CacheLayer.Add(CHNLSVC.General.IsSaleFigureRoundUp(Session["UserCompanyName"].ToString()), CacheLayer.Key.IsSaleValueRoundUp.ToString());

            CacheLayer.Add(CHNLSVC.General.GetCompByCode(Session["UserCompanyName"].ToString()), CacheLayer.Key.CompanyInfor.ToString());

            //BaseCls.GlbMasterProfitCenter = null;
            //BaseCls.GlbPriceDefinitionRef = null;      

            Session["GlbDefChannel"] = string.Empty;
            Session["GlbDefSubChannel"] = string.Empty;
            BaseCls.GlbDefArea = string.Empty;
            BaseCls.GlbDefRegion = string.Empty;
            BaseCls.GlbDefZone = string.Empty;

            DataTable _pcHierarchy = CHNLSVC.General.Get_PC_Hierarchy(Session["UserCompanyName"].ToString(), BaseCls.GlbUserDefProf);
            if (_pcHierarchy.Rows.Count > 0)
            {
                foreach (DataRow r in _pcHierarchy.Rows)
                {
                    if (System.DBNull.Value != r["CHNL"]) Session["GlbDefChannel"] = (string)r["CHNL"];
                    if (System.DBNull.Value != r["SCHNL"]) Session["GlbDefSubChannel"] = (string)r["SCHNL"];
        ...
        {
            BaseCls.GlbDefaultBin = CHNLSVC.Inventory.Get_default_binCD(Session["UserCompanyName"].ToString(), Session["UserDefLoca"].ToString());
            //CacheLayer.Clear(CacheLayer.Key.CompanyItemStatus.ToString());
            //CacheLayer.Add(CHNLSVC.Inventory.GetAllCompanyStatus(Session["UserCompanyName"].ToString()), CacheLayer.Key.CompanyItemStatus.ToString());

            CacheLayer.Clear(CacheLayer.Key.ChannelParameter.ToString());

            MasterLocation oLocation = CHNLSVC.General.GetLocationByLocCode(Session["UserCompanyName"].ToString(), Session["UserDefLoca"].ToString());
            if (oLocation.Ml_loc_tp.ToUpper() == "SERC".ToString())
            {
                Service_Chanal_parameter oChnlPara = CHNLSVC.General.GetChannelParamers(Session["UserCompanyName"].ToString(), Session["UserDefLoca"].ToString());
                if (oChnlPara != null)
                {
        ...
        {
            BaseCls.GlbIsSaleFigureRoundup = CHNLSVC.General.IsSaleFigureRoundUp(Session["UserCompanyName"].ToString());
        }
        ...
        public void RequestApprovalCycleDefinition(bool _isLocation, HirePurchasModuleApprovalCode _modulecode, DateTime _date, string _destinationcompany, string _destinationlocation, SalesPriorityHierarchyCategory _category, SalesPriorityHierarchyType _type)
        {
            ...
            if (_isLocation)
                _obj = CHNLSVC.Security.GetApproveCycleDefinitionDataforLocation(Session["UserCompanyName"].ToString(), Session["UserDefLoca"].ToString(), Convert.ToString(_modulecode), _date, _destinationcompany, _destinationlocation, Convert.ToString(_category), Convert.ToString(_type));
            else
                _obj = CHNLSVC.Security.GetApproveCycleDefinitionDataforProfitCenter(Session["UserCompanyName"].ToString(), BaseCls.GlbUserDefProf, Convert.ToString(_modulecode), _date, _destinationcompany, _destinationlocation, Convert.ToString(_category), Convert.ToString(_type));


            if (_isLocation)
                _obj_ = CHNLSVC.Security.GetUserRequestApprovalPermissionDataForLocation(Session["UserID"].ToString(), Convert.ToString(_modulecode), Session["UserCompanyName"].ToString(), Session["UserDefLoca"].ToString(), Convert.ToString(_category), Convert.ToString(_type));
            else
                _obj_ = CHNLSVC.Security.GetUserRequestApprovalPermissionDataForProfitCenter(Session["UserID"].ToString(), Convert.ToString(_modulecode), Session["UserCompanyName"].ToString(), BaseCls.GlbUserDefProf, Convert.ToString(_category), Convert.ToString(_type));
        ...
        public void CheckSessionIsExpired()
        {
            if (Session["UserID"].ToString() != "ADMIN")
            {
                ...
                if (CHNLSVC.Security.IsSessionExpired((string)Session["GlbUserSessionID"], Session["UserID"].ToString(), Session["UserCompanyName"].ToString(),out _msg) == true)
                {
                    // MessageBox.Show("Current Session is expired or has been closed by administrator!", "Fast Forward - SCM-II", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    BaseCls.GlbIsExit = true;
                    //Application.Exit();
        ...
        public void SetLoginCacheLayer(LoginUser _loginUSER, string _winLogonname, string _winUser)
        {
            ...
            {
                MasterLocation oLocation = CHNLSVC.General.GetLocationByLocCode(Session["UserCompanyName"].ToString(), _loginUSER.User_def_loc);
                if (oLocation.Ml_loc_tp.ToUpper() == "SERC".ToString())
                {
                    Service_Chanal_parameter oChnlPara = CHNLSVC.General.GetChannelParamers(Session["UserCompanyName"].ToString(), _loginUSER.User_def_loc);
                    if (oChnlPara != null)
                    {
            ...
            //_loginUSER.Chl_para = CHNLSVC.General.GetChannelParamers(Session["UserCompanyName"].ToString(), _loginUSER.User_def_loc);
            //CacheLayer.Add(_loginUSER.Chl_para, CacheLayer.Key.ChannelParameter.ToString());


            Session["GlbDefChannel"] = _loginUSER.Def_chnl;
            Session["GlbDefSubChannel"] = _loginUSER.Def_schnl;
            ...
            _loginUSER.User_session_id = Convert.ToString(CHNLSVC.Security.SaveLoginSession(Session["UserID"].ToString(), Session["UserCompanyName"].ToString(), (string)Session["GlbUserIP"], (string)Session["GlbHostName"], _winLogonname, _winUser));
        ...
        public bool IsUserProcessed(Int32 _seqno, string _document)
        {
            ...
            bool _isProcess = CHNLSVC.Inventory.CheckCompanyMulti(Session["UserCompanyName"].ToString());
            if (_isProcess == false) { _isUserProcessed = false; return _isUserProcessed; }

            string _error = string.Empty;
            DataTable _tbl = CHNLSVC.Inventory.GetProcessUser(_seqno, _document, Session["UserCompanyName"].ToString());
            ...
            if (string.IsNullOrEmpty(_user))
            {
                ...
                try
                {
                    Int32 _effect = CHNLSVC.Inventory.UpdateProcessUser(Session["UserID"].ToString(), _seqno, _document, Session["UserCompanyName"].ToString(), out _error);
                    if (_effect == -1)
                    {
        ...
        public decimal TaxCalculation(string _item, string _status, decimal _qty, PriceBookLevelRef _level, decimal _pbUnitPrice, decimal _discount, decimal _disRate, bool _isTaxfaction, bool taxPayable, bool vatExemptStatus, DateTime _serverDt, string taxType)
        {
            ...
            if (taxType == "QT")
            {
                ...
                _taxs = CHNLSVC.Inventory.GetItemTax(Session["UserCompanyName"].ToString(), _item, _status, string.Empty, string.Empty);
            ...
            if (taxType == "IN")
            {
                if (_priceBookLevelRef != null)
                    if (_priceBookLevelRef.Sapl_vat_calc)
                    {
                        ...
                        {
                            List<MasterItemTax> _taxs = new List<MasterItemTax>();
                            if (_isTaxfaction == false) _taxs = CHNLSVC.Sales.GetTax(Session["UserCompanyName"].ToString(), _item, _status); else _taxs = CHNLSVC.Sales.GetItemTax(Session["UserCompanyName"].ToString(), _item, _status, string.Empty, "VAT");
                            var _Tax = from _itm in _taxs
                                       select _itm;
                            foreach (MasterItemTax _one in _Tax)
                            {
                        ...
                        {
                            List<MasterItemTax> _taxs = new List<MasterItemTax>();
                            if (_isTaxfaction == false) _taxs = CHNLSVC.Sales.GetTaxEffDt(Session["UserCompanyName"].ToString(), _item, _status, DateTime.Now.Date); else _taxs = CHNLSVC.Sales.GetItemTaxEffDt(Session["UserCompanyName"].ToString(), _item, _status, string.Empty, "VAT", DateTime.Now.Date);
                            var _Tax = from _itm in _taxs
                                       select _itm;
                            if (_taxs.Count > 0)
                            {
                            ...
                            else
                            {
                                ...
                                if (_isTaxfaction == false) _taxsEffDt = CHNLSVC.Sales.GetTaxLog(Session["UserCompanyName"].ToString(), _item, _status, DateTime.Now.Date); else _taxsEffDt = CHNLSVC.Sales.GetItemTaxLog(Session["UserCompanyName"].ToString(), _item, _status, string.Empty, "VAT", DateTime.Now.Date);
