jQuery(document).ready(function () {
                        ...
                        success: function (result) {
                            if (result.login == true) {
                                if (result.success == true) {
                                    ...
                                    jQuery("#bld_package_nos").val(result.totalCont);
                                } else {
            ...
            success: function (result) {
                if (result.login == true) {
                    if (result.success == true) {
                        ...
                        if (jQuery("#Bl_h_doc_no").val() != null && jQuery("#Bl_h_doc_no").val() != "") {
    ...
    function SetBLHdrFiels(data) {
        ...
        jQuery("#Bl_load_in_tp").val(data[0].Bl_load_in_tp);
        jQuery("#Bl_load_out_tp").val(data[0].Bl_load_out_tp);
        
        if (data[0].Bl_m_doc_no != null && data[0].Bl_m_doc_no != "") {
            jQuery('#bl_manual_m_ref').attr("readonly", "true");
            jQuery('.blm-no-search').attr("disabled", "true");
            jQuery('#bl_manual_h_ref').attr("readonly", "true");
            jQuery('.blh-no-search').attr("disabled", "true");
        }
        //Carrier
        //jQuery("#Bl_consignee_cd").val(getFormatedDate1(data[0].Bl_consignee_cd));
    }
    ...
    function ClearAllFields() {
        ...
        jQuery('.blm-no-search').attr("disabled", "false");
        jQuery('#bl_manual_h_ref').attr("readonly", "false");
        jQuery('.blh-no-search').attr("disabled", "false");
    ...
    jQuery(".btn-new-cost").click(function (evt) {
        window.location.href = "/CostingProcess";
    });
    LoadContainerType();
    ...
    function validateSaveMandatory() {
        ...
        //if ((jQuery("#Bl_d_doc_no").val() == null || jQuery("#Bl_d_doc_no").val() == "") && ($('#Bl_d_doc_no').is('[disabled=disabled]')) == false) { setInfoMsg("Draft B/L ID cannot be empty"); return false; }
        //if ((jQuery("#Bl_m_doc_no").val() == null || jQuery("#Bl_m_doc_no").val() == "") && ($('#Bl_m_doc_no').is('[disabled=disabled]')) == false) { setInfoMsg("Master B/L ID cannot be empty"); return false; }
        if ((jQuery("#bl_manual_d_ref").val() == null || jQuery("#bl_manual_d_ref").val() == "") && ($('#bl_manual_d_ref').is('[disabled=disabled]')) == false) { setInfoMsg("Manual Draft B/L Ref cannot be empty"); return false; }
